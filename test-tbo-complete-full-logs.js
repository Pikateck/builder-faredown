...
async function test2_CountryList() {
  ...
  try {
    ...
    if (response.data?.Status === 1 && response.data?.Countries?.length > 0) {
      console.log('\n✅ PASS - Retrieved', response.data.Countries.length, 'countries');
      return true;
    } else {
...
async function test3_CityList() {
  ...
  try {
    ...
    console.log('\n📥 RESPONSE:');
    console.log('HTTP Status:', response.status);
    console.log('Body:', JSON.stringify(response.data, null, 2).substring(0, 2000) + '...');
    ...
    if (response.data?.Status === 1 && response.data?.Cities?.length > 0) {
      ...
      // Find Dubai
      const dubai = cities.find(c => c.Name && c.Name.toLowerCase().includes('dubai'));
      if (dubai) {
        console.log('\nDubai found:');
...
async function test4_HotelSearch(cities) {
  ...
  const noOfNights = Math.ceil((checkOutDateObj - checkInDateObj) / (1000 * 60 * 60 * 24));
  ...
  console.log('URL:', url);
  console.log('Method: POST');
  console.log('Body:', JSON.stringify({...requestBody, TokenId: tokenId.substring(0, 30) + '...'}, null, 2));
  ...
  try {
    ...
    if (statusOk && hasHotels) {
      console.log('\n✅ PASS - Retrieved', response.data.HotelResults.length, 'hotels');
      ...
      console.log('\n❌ FAIL - Status:', response.data?.Status || response.data?.ResponseStatus, '| Hotels:', hasHotels ? 'YES' : 'NO');
      return false;
    }
  } catch (error) {
    console.log('\n❌ ERROR:', error.message);
