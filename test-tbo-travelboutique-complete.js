...
async function runTest() {
  ...
  try {
    ...
    const hotels = searchResponse.data?.HotelResults || 
    ...
    if (hotels.length > 0) {
      ...
      hotels.slice(0, 3).forEach((h, i) => {
        ...
        console.log(`   Price: ${h.Price?.OfferedPrice || h.Price?.PublishedPrice} ${h.Price?.CurrencyCode}`);
      });
    } else {
      console.log('❌ No hotels found');
      console.log('\nResponse Status:', searchResponse.data?.ResponseStatus || searchResponse.data?.Status);
      console.log('Error:', searchResponse.data?.Error || searchResponse.data?.HotelSearchResult?.Error);
    }
