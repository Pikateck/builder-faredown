...
async function testScenario1() {
  ...
  try {
    const searchRes = await axios.post(`${API_BASE}/tbo/search`, {
      destination: 'Mumbai', cityId: 10449, countryCode: 'IN', checkIn: '2025-12-20', checkOut: '2025-12-22',
      rooms: [{ adults: 1, children: 0, childAges: [] }], currency: 'INR', guestNationality: 'IN'
    }, { timeout: 30000 });
    ...
    const roomRes = await axios.post(`${API_BASE}/tbo/room`, {
      ...
      checkInDate: '2025-12-20', checkOutDate: '2025-12-22', noOfRooms: 1
    }, { timeout: 30000 });
    ...
    const bookRes = await axios.post(`${API_BASE}/tbo/book`, {
      traceId: searchRes.data.traceId, resultIndex: hotel.resultIndex, hotelCode: hotel.hotelCode, hotelName: hotel.hotelName,
      bookingId: blockRes.data.bookingId, guestNationality: 'IN', noOfRooms: 1, isVoucherBooking: true,
      hotelRoomDetails: blockRes.data.hotelRoomDetails,
      hotelPassenger: [{ Title: 'Mr', FirstName: 'Rajesh', LastName: 'Kumar', PaxType: 1, Nationality: 'IN', Email: 'rajesh@example.com', Phoneno: '+919876543210' }]
    }, { timeout: 30000 });
...
testScenario1().then(r => { console.log('='.repeat(80)); console.log(JSON.stringify(r, null, 2)); process.exit(r.status === 'PASSED' ? 0 : 1); });
