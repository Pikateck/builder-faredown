  ...
  try {
    ...
    if (Array.isArray(destinations) && destinations.length > 0) {
      ...
      const dubai = destinations.find(d => 
        String(d.CityName || d.Name || d.DestinationName || '').toLowerCase().includes('dubai')
      );
    ...
    if (error.response?.status !== 404) {
      console.log("Error data:", JSON.stringify(error.response?.data, null, 2).substring(0, 500));
    }
    return null;
  }
}
...
async function main() {
  ...
  console.log("║" + " ".repeat(20) + "TEST STATIC DATA ENDPOINTS" + " ".repeat(32) + "║");
  console.log("╚" + "═".repeat(78) + "╝\n");
  ...
  if (success) {
    ...
    const destinationId = success.dubai.DestinationId || success.dubai.CityId || success.dubai.Id;
    console.log("\n✅ Dubai DestinationId:", destinationId);
    
    fs.writeFileSync("dubai-destination-success.json", JSON.stringify({
      url: success.url,
