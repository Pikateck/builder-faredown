...
async function testDubaiFiltering() {
  try {
    console.log('🧪 Testing End-to-End Dubai Filtering for October 1-5, 2025\n');
    ...
    if (result.rows.length > 0) {
      ...
      result.rows.forEach((pkg, index) => {
        console.log(`${index + 1}. ${pkg.title} (${pkg.package_category || pkg.category})`);
        console.log(`   📍 ${pkg.city_name}, ${pkg.country_name} (${pkg.region_name})`);
        console.log(`   💰 ₹${pkg.from_price} | Rating: ${pkg.rating || 'N/A'}`);
      ...
      const nonDubaiPackages = result.rows.filter(pkg => !pkg.city_name || pkg.city_name !== 'Dubai');
      ...
      const categories = [...new Set(result.rows.map(pkg => pkg.package_category || pkg.category))];
      ...
      console.log(`DEBUG: Found ${departuresCheck.rows[0].count} departures for Dubai Oct 1-5`);
    }
