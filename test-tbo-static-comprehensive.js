  ...
  try {
    ...
    console.log(
      ...
      response.data?.TokenId
        ? "✅ " + response.data.TokenId.substring(0, 40) + "..."
        : "❌ MISSING",
    );
...
async function testJSONWithToken(host, endpoint, tokenId) {
  ...
  console.log(
    "  Payload:",
    JSON.stringify(
      { ...request, TokenId: tokenId.substring(0, 40) + "..." },
      null,
      2,
    ),
  );
...
async function main() {
  ...
  console.log(
    "║" +
      " ".repeat(20) +
      "TBO STATIC DATA COMPREHENSIVE TEST" +
      " ".repeat(24) +
      "║",
  );
  console.log("╚" + "═".repeat(78) + "╝");
  ...
  console.log(
    "  Static Password:",
    CONFIG.staticPassword
      ? "***" +
          CONFIG.staticPassword.substring(CONFIG.staticPassword.length - 4)
      : "MISSING",
  );
  ...
  console.log(
    "  Proxy:",
    CONFIG.useProxy ? CONFIG.proxy?.substring(0, 30) + "..." : "Disabled",
  );
  ...
  const endpoints = ["DestinationCityList", "CityList", "CountryList"];
  ...
  if (results.length > 0) {
    ...
    console.log(
      `   node test-tbo-hotel-search-with-cityid.js ${results[0].dubai.CityId || results[0].dubai.Id}`,
    );
  } else {
    console.log(
    ...
    console.log("\n📞 RECOMMENDATION:");
    console.log(
      "  Since API methods are failing, the city list data must be obtained from:",
    );
    console.log("  1. TBO B2B Portal (requires manual login)");
    console.log(
      "     - Log in at: https://b2b.travelboutiqueonline.com (or similar)",
    );
