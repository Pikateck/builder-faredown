...
async function testCountryListPaths() {
  ...
  console.log(JSON.stringify({...requestBody, TokenId: tokenId.substring(0, 30) + '...'}, null, 2));
  console.log('');
...
function printSummary(countryResults, cityResults) {
  ...
  const workingCountry = countryResults.filter(r => r.success && r.status === 200);
  ...
  console.log(`  Success: ${workingCountry.length + workingCity.length} / ${countryResults.length + cityResults.length}`);
  console.log('═'.repeat(80));
  console.log('');
}
