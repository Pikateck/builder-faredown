...
async function testHotelBookingSubdomain() {
  console.log("Testing hotelbooking.travelboutiqueonline.com for GetHotelResult...");
  console.log("");
  ...
  for (const url of searchPaths) {
    try {
      ...
      if (response.data.HotelSearchResult) {
        ...
        const hotelCount = response.data.HotelSearchResult.HotelResults?.length || 0;
        ...
        if (status === 1 && hotelCount > 0) {
          ...
          response.data.HotelSearchResult.HotelResults.slice(0, 3).forEach((h, i) => {
