...
const API_BASE = process.env.API_BASE_URL || 'https://builder-faredown-pricing.onrender.com';
...
async function testTBOIntegration() {
  ...
  try {
    ...
    const searchResult = await makeRequest(`/api/flights/search?${searchParams}`);
    ...
    if (searchResult.data.meta) {
      ...
      if (searchResult.data.meta.suppliers) {
        console.log(`\n   📊 Supplier Metrics:`);
        Object.entries(searchResult.data.meta.suppliers).forEach(([supplier, metrics]) => {
    ...
    if (searchResult.data.data && searchResult.data.data.length > 0) {
      ...
      console.log(`   - Route: ${flight.origin || flight.departure?.code} → ${flight.destination || flight.arrival?.code}`);
      console.log(`   - Price: ${flight.price?.final || flight.price?.amount || flight.price}`);
      console.log(`   - Supplier: ${flight.supplier || 'N/A'}`);
    }
    
    // Test 3: Verify Supplier-Aware Markup
    console.log('\n📋 Test 3: Supplier-Aware Markup Rules');
    const markupResult = await makeRequest('/api/pricing/markup-rules?supplier=tbo&limit=5');
    console.log(`✅ Status: ${markupResult.status}`);
    if (markupResult.data.data) {
      console.log(`   Total TBO Markup Rules: ${markupResult.data.total || markupResult.data.data.length}`);
    }
    
    // Test 4: Verify Supplier-Aware Promo Codes
    console.log('\n📋 Test 4: Supplier-Aware Promo Codes');
    const promoResult = await makeRequest('/api/pricing/promo-codes?supplier=tbo&limit=5');
    console.log(`✅ Status: ${promoResult.status}`);
    if (promoResult.data.data) {
      console.log(`   Total TBO Promo Codes: ${promoResult.data.total || promoResult.data.data.length}`);
    }
