...
async function runTest() {
  ...
  console.log('Body:', JSON.stringify({...searchRequest, TokenId: tokenId.substring(0, 30) + '...'}, null, 2));
  console.log('');
  ...
  try {
    ...
    console.log('Response Status:', searchResponse.data?.ResponseStatus || searchResponse.data?.Status);
    ...
    if (searchResponse.data?.HotelResults?.length > 0) {
      ...
      console.log('Full Response:', JSON.stringify(searchResponse.data, null, 2).substring(0, 1000));
    }
