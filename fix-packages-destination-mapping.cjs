...
async function fixPackagesDestinationMapping() {
  ...
  try {
    ...
    console.log('📍 Step 1: Updating existing packages with destination linkage...');
    ...
    for (const region of majorRegions) {
      const regionResult = await client.query('SELECT id FROM regions WHERE name = $1', [region.name]);
      ...
      for (const packageType of packageTypes) {
        ...
        if (existing.rows[0].count === '0') {
          ...
          const basePrice = packageType === 'luxury' ? 125000 : (packageType === 'standard' ? 75000 : 45000);
          const duration = packageType === 'luxury' ? 7 : (packageType === 'standard' ? 5 : 4);
          
          // Get city_id if specified
          let cityId = null;
          if (region.city_name) {
            const cityResult = await client.query('SELECT id FROM cities WHERE name = $1', [region.city_name]);
          ...
          if (region.name === 'Middle East') {
            const countryResult = await client.query('SELECT id FROM countries WHERE name = $1', ['United Arab Emirates']);
          ...
          `, [
            ...
            'active',
            packageType === 'luxury',
            JSON.stringify(packageType === 'luxury' 
              ? ["5-star accommodation", "Private transfers", "Personal guide", "Premium experiences"]
              : packageType === 'standard' 
              ? ["3-star accommodation", "Group transfers", "Guided tours", "Cultural experiences"]
              : ["Budget accommodation", "Local transport", "Basic sightseeing", "Authentic experiences"]
            ),
            JSON.stringify(["International flights", "Personal expenses", "Travel insurance", "Optional activities"])
          ]);
    ...
    for (const pkg of packagesWithoutDepartures.rows) {
      for (const depDate of departureDates) {
        ...
        `, [
          ...
          pkg.base_price_pp, 5000, pkg.base_price_pp * 0.8, 0,
    ...
    verification.rows.forEach(row => {
      console.log(`${row.region_name}: ${row.total_packages} total (${row.luxury_count} luxury, ${row.standard_count} standard, ${row.budget_count} budget)`);
    });
