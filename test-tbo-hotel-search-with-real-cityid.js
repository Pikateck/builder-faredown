...
async function authenticate() {
  ...
  try {
    ...
    console.log("TokenId:", response.data?.TokenId ? "✅ " + response.data.TokenId : "❌ MISSING");
    console.log("Member:", response.data?.Member?.FirstName, response.data?.Member?.LastName);
...
async function searchHotels(tokenId) {
  ...
  console.log("\nKey Parameters:");
  console.log("  CityId:", request.CityId, "(Dubai - from GetDestinationSearchStaticData)");
  ...
  try {
    ...
    if (response.data?.HotelResults?.length > 0) {
      console.log("\n✅ SUCCESS! Found", response.data.HotelResults.length, "hotels in Dubai");
      ...
      // Save full response
      fs.writeFileSync("tbo-dubai-hotel-search-SUCCESS.json", JSON.stringify({
        request: {
          ...request,
          TokenId: "***REDACTED***"
        },
      ...
      fs.writeFileSync("tbo-dubai-hotel-search-no-results.json", JSON.stringify({
        request: request,
        response: response.data,
        timestamp: new Date().toISOString()
      }, null, 2));
    ...
    fs.writeFileSync("tbo-dubai-hotel-search-ERROR.json", JSON.stringify({
      request: request,
...
async function main() {
  console.log("\n╔" + "═".repeat(78) + "╗");
  console.log("║" + " ".repeat(18) + "TBO HOTEL SEARCH - DUBAI (REAL CITYID)" + " ".repeat(22) + "║");
  console.log("╚" + "═".repeat(78) + "╝\n");
  
  console.log("Configuration:");
  console.log("  Dubai CityId:", CONFIG.dubaicityId, "(from GetDestinationSearchStaticData)");
  ...
  if (hotels && hotels.HotelResults?.length > 0) {
    ...
    console.log("\n✅ Auth: SUCCESS");
    console.log("✅ GetDestinationSearchStaticData: SUCCESS (DestinationId: 115936)");
    console.log("✅ GetHotelResult: SUCCESS (" + hotels.HotelResults.length + " hotels)");
    console.log("\n📊 INTEGRATION STATUS: READY FOR PRODUCTION");
    console.log("\n🚀 NEXT STEPS:");
    console.log("  1. Wire Dubai CityId (115936) into Faredown hotel search");
    console.log("  2. Implement city autocomplete using GetDestinationSearchStaticData");
    console.log("  3. Test GetHotelRoom (room details)");
    console.log("  4. Test BlockRoom (pre-book)");
    console.log("  5. Test Book (final booking)");
    console.log("\n📁 FILES CREATED:");
    console.log("  - dubai-destination-success.json (static data with all UAE cities)");
    console.log("  - tbo-dubai-hotel-search-SUCCESS.json (full hotel search results)");
  } else {
    console.log("\n❌ HOTEL SEARCH FAILED");
