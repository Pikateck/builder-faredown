...
async function testApiEndpoint() {
  ...
  const baseUrl = 'https://55e69d5755db4519a9295a29a1a55930-aaf2790235d34f3ab48afa56a.fly.dev';
  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      if (data.success && data.data && data.data.packages) {
        ...
        console.log(`✅ Response successful: ${packages.length} packages found`);
        
        packages.forEach((pkg, index) => {
          console.log(`${index + 1}. ${pkg.title} (${pkg.package_category || 'Unknown'})`);
          console.log(`   📍 ${pkg.city_name || 'Unknown'}, ${pkg.country_name || 'Unknown'}`);
          console.log(`   💰 ₹${pkg.base_price_pp?.toLocaleString() || pkg.from_price?.toLocaleString() || 'N/A'}`);
        });
        
        // Check if all packages are Dubai packages for Dubai searches
        if (testCase.name.includes('Dubai')) {
          const nonDubaiPackages = packages.filter(pkg => 
            pkg.city_name && pkg.city_name.toLowerCase() !== 'dubai'
          );
          
          if (nonDubaiPackages.length > 0) {
            console.log(`\n❌ FILTERING ISSUE: Found ${nonDubaiPackages.length} non-Dubai packages:`);
            nonDubaiPackages.forEach(pkg => {
              console.log(`   ❌ ${pkg.title} - ${pkg.city_name}, ${pkg.country_name}`);
            });
          } else {
  ...
  console.log('\n🏁 API TEST SUMMARY');
  console.log('==================');
  console.log('If Dubai tests show non-Dubai packages, the issue is in the API filtering logic.');
  console.log('If all tests fail, the issue might be with the API server or URL configuration.');
}
