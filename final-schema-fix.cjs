...
async function finalSchemaFix() {
  ...
  try {
    ...
    await client.query('UPDATE packages SET region_id = NULL, country_id = NULL, city_id = NULL');
    
    // Drop existing constraints if any
    await client.query('ALTER TABLE packages DROP CONSTRAINT IF EXISTS packages_region_id_fkey');
    await client.query('ALTER TABLE packages DROP CONSTRAINT IF EXISTS packages_country_id_fkey');
    await client.query('ALTER TABLE packages DROP CONSTRAINT IF EXISTS packages_city_id_fkey');
    
    // Change data types to UUID
    await client.query('ALTER TABLE packages ALTER COLUMN region_id TYPE UUID USING NULL');
    await client.query('ALTER TABLE packages ALTER COLUMN country_id TYPE UUID USING NULL');
    await client.query('ALTER TABLE packages ALTER COLUMN city_id TYPE UUID USING NULL');
    ...
    // Get Dubai city UUID
    const dubaiCity = await client.query('SELECT id FROM cities WHERE name = $1', ['Dubai']);
    const uaeCountry = await client.query('SELECT id FROM countries WHERE name = $1', ['United Arab Emirates']);
    const middleEastRegion = await client.query('SELECT id FROM regions WHERE name = $1', ['Middle East']);
    
    if (dubaiCity.rows.length > 0 && uaeCountry.rows.length > 0 && middleEastRegion.rows.length > 0) {
    ...
    const europeRegion = await client.query('SELECT id FROM regions WHERE name = $1', ['Europe']);
    ...
    const asiaRegion = await client.query('SELECT id FROM regions WHERE name = $1', ['Asia']);
    ...
    const indiaRegion = await client.query('SELECT id FROM regions WHERE name = $1', ['India']);
    ...
    try {
      await client.query('ALTER TABLE packages ADD COLUMN IF NOT EXISTS package_category VARCHAR(50)');
    } catch (err) {
      // Column already exists
    }
    
    // Set package categories
    await client.query(`UPDATE packages SET package_category = 'luxury' WHERE LOWER(title) LIKE '%luxury%'`);
    await client.query(`UPDATE packages SET package_category = 'explorer' WHERE LOWER(title) LIKE '%explorer%' OR LOWER(title) LIKE '%city%'`);
    await client.query(`UPDATE packages SET package_category = 'adventure' WHERE LOWER(title) LIKE '%adventure%' OR LOWER(title) LIKE '%trek%'`);
    await client.query(`UPDATE packages SET package_category = 'cultural' WHERE LOWER(title) LIKE '%highlights%' OR LOWER(title) LIKE '%triangle%' OR LOWER(title) LIKE '%essentials%'`);
    await client.query(`UPDATE packages SET package_category = 'beach' WHERE LOWER(title) LIKE '%beach%' OR LOWER(title) LIKE '%paradise%'`);
    await client.query(`UPDATE packages SET package_category = 'romantic' WHERE LOWER(title) LIKE '%romantic%' OR LOWER(title) LIKE '%getaway%'`);
    await client.query(`UPDATE packages SET package_category = 'family' WHERE LOWER(title) LIKE '%family%'`);
    await client.query(`UPDATE packages SET package_category = 'standard' WHERE package_category IS NULL`);
    ...
    if (dubaiCity.rows.length > 0) {
      ...
      const existingCategories = existingDubai.rows.map(row => row.package_category);
      ...
      for (const req of requiredCategories) {
        if (!existingCategories.includes(req.category)) {
          ...
          `, [
            ...
            uaeCountryId,
            dubaiCityId,
            req.category === 'luxury' ? 7 : (req.category === 'standard' ? 5 : 4),
            req.category === 'luxury' ? 6 : (req.category === 'standard' ? 4 : 3),
            ...
            req.category === 'luxury',
            JSON.stringify([
              req.category === 'luxury' ? "5-star accommodation" : req.category === 'standard' ? "4-star accommodation" : "3-star accommodation",
              "Airport transfers",
              "City tour",
              "Desert safari"
            ]),
    ...
    dubaiPackages.rows.forEach(row => {
      ...
      console.log(`  📍 ${row.city_name}, ${row.country_name} (${row.region_name})`);
    });
