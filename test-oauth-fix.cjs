...
async function testOAuthFix() {
  ...
  try {
    ...
    if (
      urlResponse.data.success &&
      urlResponse.data.url &&
      urlResponse.data.state
    ) {
    ...
    try {
      ...
      console.log(
        "✅ Callback route is accessible and handles errors properly",
      );
    } catch (error) {
    ...
    console.log(
      "❌ OLD: 1-second popup check conflicted with 1-second auto-close",
    );
