...
const API_BASE = 'https://55e69d5755db4519a9295a29a1a55930-aaf2790235d34f3ab48afa56a.fly.dev/api';
...
async function testPackagesAPI() {
  ...
  const tests = [
    ...
    },
    {
      ...
      expectation: 'Should return packages with "cultural" in title/description'
    }
  ];
    ...
    try {
      ...
      if (result.status === 200) {
        if (result.data.success && result.data.data) {
          const packages = Array.isArray(result.data.data) ? result.data.data : result.data.data.packages || [];
          ...
          if (packages.length > 0) {
            ...
            packages.slice(0, 3).forEach((pkg, index) => {
              console.log(`   ${index + 1}. ${pkg.title} - ₹${pkg.base_price_pp?.toLocaleString() || 'N/A'} (${pkg.package_category || 'N/A'})`);
            });
          }
        } else {
  ...
  try {
    const detailsResult = await makeRequest('/api/packages/dubai-luxury-experience');
    if (detailsResult.status === 200) {
      ...
      console.log(`❌ Package details failed with status ${detailsResult.status}`);
    }
  } catch (error) {
