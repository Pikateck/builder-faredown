...
async function verifyAndEnableRateHawk() {
  try {
    ...
    suppliersResult.rows.forEach(s => {
      console.log(`   - ${s.code}: ${s.name} (${s.product_type}) - Enabled: ${s.is_enabled ? '✅' : '❌'} [${s.environment}] Last sync: ${s.last_sync_at || 'Never'}`);
    });
    ...
    console.log(`   - RATEHAWK_BASE_URL: ${process.env.RATEHAWK_BASE_URL || 'https://api.worldota.net/api/b2b/v3/'} ✅`);
