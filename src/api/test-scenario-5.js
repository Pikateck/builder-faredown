...
async function testScenario5() {
  console.log('\n' + '='.repeat(80));
  console.log('SCENARIO 5: International (Paris, 1 Room, 1 Adult, US)');
  console.log('='.repeat(80));
  try {
    const searchRes = await axios.post(`${API_BASE}/tbo/search`, {
      destination: 'Paris', cityId: 3, countryCode: 'FR', checkIn: '2026-01-05', checkOut: '2026-01-07',
      rooms: [{ adults: 1, children: 0, childAges: [] }], currency: 'EUR', guestNationality: 'US'
    }, { timeout: 30000 });
    ...
    const roomRes = await axios.post(`${API_BASE}/tbo/room`, {
      ...
      checkInDate: '2026-01-05', checkOutDate: '2026-01-07', noOfRooms: 1
    }, { timeout: 30000 });
    if (!roomRes.data.success) throw new Error('Room details failed');
    const blockRes = await axios.post(`${API_BASE}/tbo/block`, {
      traceId: searchRes.data.traceId, resultIndex: hotel.resultIndex, hotelCode: hotel.hotelCode, hotelName: hotel.hotelName,
      guestNationality: 'US', noOfRooms: 1, isVoucherBooking: true, hotelRoomDetails: roomRes.data.hotelRoomDetails
    }, { timeout: 30000 });
    if (!blockRes.data.success) throw new Error('Block failed');
    const bookRes = await axios.post(`${API_BASE}/tbo/book`, {
      traceId: searchRes.data.traceId, resultIndex: hotel.resultIndex, hotelCode: hotel.hotelCode, hotelName: hotel.hotelName,
      bookingId: blockRes.data.bookingId, guestNationality: 'US', noOfRooms: 1, isVoucherBooking: true,
      hotelRoomDetails: blockRes.data.hotelRoomDetails,
      hotelPassenger: [{ Title: 'Mr', FirstName: 'John', LastName: 'Smith', PaxType: 1, Nationality: 'US', Email: 'john@example.com', Phoneno: '+12025551234' }]
    }, { timeout: 30000 });
...
testScenario5().then(result => { console.log('='.repeat(80)); console.log(JSON.stringify(result, null, 2)); process.exit(result.status === 'PASSED' ? 0 : 1); });
