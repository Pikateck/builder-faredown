...
(async () => {
  ...
  try {
    ...
    result.rows.forEach((row, index) => {
      ...
      console.log(
        `   Airline: ${row.airline_code} ${row.airline_code === "ALL" ? "✅" : "❌"}`,
      );
      console.log(
        `   Route From: ${row.route_from} ${row.route_from === "ALL" ? "✅" : "❌"}`,
      );
      console.log(
        `   Route To: ${row.route_to} ${row.route_to === "ALL" ? "✅" : "❌"}`,
      );
      console.log(
        `   Origin IATA: ${row.origin_iata} ${row.origin_iata === "ALL" ? "✅" : "❌"}`,
      );
      console.log(
        `   Dest IATA: ${row.dest_iata} ${row.dest_iata === "ALL" ? "✅" : "❌"}`,
      );
    ...
    const allGlobal = result.rows.every(
      (row) =>
    ...
    const correctNaming = result.rows.every(
      (row) =>
    ...
    console.log(
      '   Class Column: "All – [Economy/Premium Economy/Business/First] Class"',
    );
