...
async function checkDatabaseStructure() {
  try {
    ...
    try {
      ...
      if (packagesCount.rows[0].count > 0) {
        ...
        samplePackages.rows.forEach(pkg => {
          console.log(`- ${pkg.title} (region: ${pkg.region_id}, country: ${pkg.country_id}, city: ${pkg.city_id}, status: ${pkg.status})`);
        });
      }
    } catch (err) {
    ...
    try {
      const regionsCount = await pool.query('SELECT COUNT(*) as count FROM regions');
      ...
      // Check for Dubai specifically
      const dubaiCheck = await pool.query(`
        SELECT c.id, c.name, co.name as country_name, r.name as region_name
        FROM cities c
        JOIN countries co ON c.country_id = co.id
        LEFT JOIN regions r ON c.region_id = r.id
        WHERE c.name ILIKE '%dubai%'
      `);
      
      if (dubaiCheck.rows.length > 0) {
        console.log('\nDUBAI FOUND:');
        dubaiCheck.rows.forEach(city => {
          console.log(`- ${city.name}, ${city.country_name} (Region: ${city.region_name})`);
        });
      } else {
        console.log('\nDUBAI NOT FOUND IN CITIES');
      }
      
    } catch (err) {
      console.log('\n=== DESTINATIONS CHECK ERROR ===');
      console.log(err.message);
    }
