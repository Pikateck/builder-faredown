...
export async function adminApiBypass<T = any>(
  ...
  console.log('🚀 BYPASS: Using Image object for API call', { endpoint, options });
...
export const adminApi = {
  get: async <T = any>(endpoint: string, params?: Record<string, any>, headers?: Record<string, string>): Promise<T> => {
    const url = new URL(`https://builder-faredown-pricing.onrender.com/api${endpoint}`);
    if (params) {
  ...
  post: async <T = any>(endpoint: string, data?: any, headers?: Record<string, string>): Promise<T> => {
  ...
  put: async <T = any>(endpoint: string, data?: any, headers?: Record<string, string>): Promise<T> => {
    return adminApiBypass(endpoint, {
      method: 'PUT',
      headers: headers || {},
      body: data
  ...
  delete: async <T = any>(endpoint: string, headers?: Record<string, string>): Promise<T> => {
    return adminApiBypass(endpoint, {
      method: 'DELETE',
      headers: headers || {}
    });
