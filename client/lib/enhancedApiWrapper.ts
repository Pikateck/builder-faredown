...
export abstract class EnhancedApiService {
  ...
  protected logServiceEvent(level: 'info' | 'warn' | 'error', message: string, data?: any) {
    const logData = {
    ...
    if (API_CONFIG.IS_PRODUCTION) {
      ...
      console[level](`[FAREDOWN_${this.serviceName.toUpperCase()}] ${message}`, logData);
    } else {
      ...
      if (response && typeof response === 'object') {
        if ('success' in response && 'data' in response) {
          if ((response as any).success) {
            ...
            this.logServiceEvent('warn', `${operationDesc} returned success=false`, response);
            return fallbackData || response;
          }
        }
        
        if ('data' in response && typeof (response as any).data === 'object' && 'success' in (response as any).data) {
          const innerData = (response as any).data;
          if (innerData.success) {
            this.logServiceEvent('info', `${operationDesc} successful (nested)`);
            return innerData.data;
          } else {
            this.logServiceEvent('warn', `${operationDesc} nested success=false`, response);
            return fallbackData || response;
          }
        }
      }
  ...
  protected async safePost<T>(
    ...
    return this.safeApiCall(
      () => apiClient.post<T>(`${this.baseEndpoint}${endpoint}`, data, customHeaders),
      fallbackData,
...
export function useApiHealth(services: string[]) {
  const [healthStatus, setHealthStatus] = React.useState<Record<string, boolean>>({});
