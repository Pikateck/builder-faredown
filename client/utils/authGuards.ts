...
export const storeResumeContext = <T>(contextId: string, type: PostAuthIntent, payload: T): void => {
...
export const getResumeContext = <T>(contextId: string): ResumeContext<T> | null => {
  try {
...
export const useAuthGuard = () => {
  ...
  const requireAuth = (intent: PostAuthIntent, payload: any, fallbackPath = '/') => {
    if (isLoggedIn && user) {
...
export const resumePostAuth = async (nextPath: string, intent: PostAuthIntent) => {
  try {
    ...
    if (context.type !== intent) {
      console.warn('Context type mismatch', { expected: intent, actual: context.type });
      return false;
    }
...
export const preflightBargain = async (searchContext: SearchContext): Promise<boolean> => {
  try {
