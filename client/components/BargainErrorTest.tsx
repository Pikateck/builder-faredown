...
export default function BargainErrorTest() {
  ...
  const handleAccept = (finalPrice: number, orderRef: string, holdData?: any) => {
    console.log('✅ Bargain accepted:', { finalPrice, orderRef, holdData });
    alert(`Bargain accepted! Final price: ₹${finalPrice}\nOrder Ref: ${orderRef}\nHold Status: ${holdData?.isHeld ? 'Held' : 'Not held'}`);
  ...
  return (
    <div className="p-4 border rounded-lg m-4">
      ...
      <p className="text-sm text-gray-600 mb-4">
        Test the ConversationalBargainModal to verify it handles API errors gracefully.
        This should work even when the backend is offline.
      </p>
