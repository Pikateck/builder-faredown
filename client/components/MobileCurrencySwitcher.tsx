...
const MobileCurrencySwitcher: React.FC<MobileCurrencySwitcherProps> = ({ 
  ...
  const convertPrice = (amount: number, fromCurrency: string = 'INR', toCurrency: string = currency): number => {
    const fromRate = currencies.find(c => c.code === fromCurrency)?.rate || 1;
    const toRate = currencies.find(c => c.code === toCurrency)?.rate || 1;
    return amount * (toRate / fromRate);
  };

  const formatPrice = (amount: number, currencyCode: string = currency): string => {
    const curr = currencies.find(c => c.code === currencyCode) || currentCurrency;
