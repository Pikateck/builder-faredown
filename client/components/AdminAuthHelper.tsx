  ...
  const testAuth = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setMessage('✅ Authentication test successful! Admin APIs are working.');
      } else {
        setMessage(`❌ Authentication test failed: ${data.error?.message || 'Unknown error'}`);
      }
    } catch (error) {
    ...
    <Card className="max-w-2xl mx-auto">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="text-xs text-gray-500 space-y-1">
          ...
          <p>3. The token is automatically saved to localStorage for API calls</p>
