...
export function PackagesSearchForm() {
  ...
  // Popular destinations (like Sightseeing)
  const popularDestinations = [
    { name: "Dubai, United Arab Emirates", code: "DXB", type: "city" as const },
    { name: "Paris, France", code: "PAR", type: "city" as const },
    { name: "London, United Kingdom", code: "LON", type: "city" as const },
    { name: "Rome, Italy", code: "ROM", type: "city" as const },
    { name: "Barcelona, Spain", code: "BCN", type: "city" as const },
    { name: "New York, United States", code: "NYC", type: "city" as const },
    { name: "Bangkok, Thailand", code: "BKK", type: "city" as const },
    { name: "Singapore", code: "SIN", type: "city" as const },
    { name: "Tokyo, Japan", code: "TYO", type: "city" as const },
    { name: "Sydney, Australia", code: "SYD", type: "city" as const },
    { name: "Europe", code: "EUR", type: "region" as const },
    { name: "Asia", code: "ASI", type: "region" as const },
  ];
  ...
  const performSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      console.log(`🔍 Frontend search for "${query}" took ${searchTime}ms`);
      ...
      const searchTime = Date.now() - searchStart;
      console.error(`❌ Frontend search error for "${query}" after ${searchTime}ms:`, error);
      setSearchResults([]);
    }
    setLoadingSearch(false);
  }, []);

  // Handle destination input change with faster debouncing
  ...
  const handleDestinationSelect = (destination: DestinationSearchResult | typeof popularDestinations[0]) => {
    const selected: SelectedDestination = {
      type: destination.type,
      id: 'id' in destination ? destination.id : '',
      label: 'label' in destination ? destination.label : destination.name,
      region: 'region' in destination ? destination.region : '',
      country: 'country' in destination ? destination.country : undefined,
    };
  ...
  // Get destination code for display
  const getDestinationCode = (destination: SelectedDestination | null) => {
    if (!destination) return "";
    
    // For popular destinations, try to match the code
    const popular = popularDestinations.find(p => 
      p.name.toLowerCase().includes(destination.label.toLowerCase()) ||
      destination.label.toLowerCase().includes(p.name.toLowerCase())
    );
    
    if (popular) return popular.code;
    
    // Generate a code from the destination name
    const nameParts = destination.label.split(',')[0].trim();
    return nameParts.substring(0, 3).toUpperCase();
  };
