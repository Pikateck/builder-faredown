...
export function CountrySelect({
  ...
  const filteredCountries = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = countries.filter(country => 
        ...
        (country.iso3_code && country.iso3_code.toLowerCase().includes(query))
      );
    }
  ...
  const formatCountryDisplay = useCallback((country: CountryOption, showCode = showCodes) => {
    const parts = [];
  ...
  if (!searchable) {
    return (
      <div className="space-y-1">
        ...
        >
          <SelectTrigger className={cn(sizeClasses[size], className, error && 'border-red-500')}>
        ...
        <PopoverContent className="w-full p-0" align="start">
          <Command>
            ...
            <CommandList>
              ...
              {!loading && !fetchError && popularCountries.length > 0 && prioritizePopular && !popularOnly && (
                <CommandGroup heading="Popular Countries">
                  {popularCountries
                    ...
                    .map((country) => (
                      ...
                      >
                        {formatCountryDisplay(country)}
                        <span className="ml-auto text-xs text-blue-600">Popular</span>
                      </CommandItem>
                    ))}
                </CommandGroup>
              )}
              ...
              {!loading && !fetchError && groupByContinent ? (
                ...
                !loading && !fetchError && (
                  <CommandGroup heading={popularOnly ? "Popular Countries" : "All Countries"}>
                    {filteredCountries
                      .filter(country => 
                        !prioritizePopular || 
                        popularOnly || 
