  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "forgot-password") {
        ...
        login({
          id: response.user.id,
          name: response.user.firstName ? `${response.user.firstName} ${response.user.lastName}` : response.user.email,
      ...
      if (error.response?.status === 401) {
        ...
        setError(`${mode === "forgot-password" ? "Password reset failed" : "Authentication failed"}: ${error.message || "Please try again."}`);
      }
    } finally {
      setIsLoading(false);
    }
