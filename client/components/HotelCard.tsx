...
export function HotelCard({
  ...
  const discount = Math.round(
    ((hotel.originalPrice - hotel.currentPrice) / hotel.originalPrice) * 100,
  );
    ...
    <Card className="overflow-hidden hover:shadow-xl transition-all duration-300 border-0 shadow-lg group touch-manipulation">
      <div className="flex flex-col sm:flex-row">
        ...
        <div className="relative sm:w-64 md:w-80 h-48 sm:h-64 md:h-80 flex-shrink-0">
          ...
          {/* Discount Badge */}
          {discount > 0 && (
            <Badge className="absolute top-2 sm:top-4 left-2 sm:left-4 bg-red-500 text-white text-xs sm:text-sm">
              -{discount}%
            </Badge>
          )}
