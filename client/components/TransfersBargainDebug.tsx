...
export function TransfersBargainDebug() {
  ...
  const testBargainAPI = async () => {
    ...
    try {
      ...
      if (healthResponse.ok) {
        ...
        addLog("error", "❌ Health check failed", { status: healthResponse.status });
      }
      ...
      if (startResponse.ok) {
        ...
        addLog("error", "❌ Session start failed", { status: startResponse.status });
      }
  ...
  const testFallbackLogic = () => {
    ...
    if (userOffer >= minSellingPrice) {
      ...
      addLog("error", "❌ Offer rejected - below minimum", { minRequired: minSellingPrice });
    }
  };
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-lg">
      <h3 className="text-lg font-semibold mb-4">Transfers Bargain Debug Console</h3>
