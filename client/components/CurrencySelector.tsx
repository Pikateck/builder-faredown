      ...
      <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" className="h-9 px-3">
            ...
            <span className="hidden sm:inline mr-1">{selectedCurrency.flag}</span>
            <span className="font-medium">{selectedCurrency.code}</span>
        ...
        <DropdownMenuContent align="end" className="w-64">
          <DropdownMenuLabel className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {isLoading && <Badge variant="secondary" className="text-xs">Updating...</Badge>}
              <Button
                variant="ghost"
                size="sm"
                className="h-6 w-6 p-0"
                ...
                <div>
                  ...
                  {currency.code !== 'INR' && (
                    <div className="text-xs text-blue-600">
                      1 INR = {currency.rate.toFixed(currency.decimalPlaces)} {currency.code}
                    </div>
                  )}
                </div>
              </div>
      ...
      <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" className="w-full justify-between">
            <div className="flex items-center">
              {showFlag && <span className="mr-2">{selectedCurrency.flag}</span>}
              <span className="font-medium">{selectedCurrency.code}</span>
              ...
              >
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    {currency.code !== 'INR' && (
                      <div className="text-xs text-blue-600">
                        Rate: {currency.rate.toFixed(currency.decimalPlaces > 2 ? 4 : currency.decimalPlaces)}
                      </div>
                    )}
                  </div>
                </div>
