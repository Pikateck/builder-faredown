...
export const TransferInvoice: React.FC<TransferInvoiceProps> = ({
  ...
  const discountAmount = booking.discountAmount ? parseAmount(booking.discountAmount) : 0;
      ...
      <div className="page-content">
        ...
        <div className="border-b-2 border-[#003580] pb-6 mb-6">
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-3xl font-bold text-[#003580] mb-2">FAREDOWN</h1>
            ...
            <div className="text-right">
              ...
              <p className="text-sm text-gray-600">Invoice No: {booking.invoiceNumber}</p>
              <p className="text-sm text-gray-600">Date: {formatDate(booking.invoiceDate)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div>
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <p className="font-semibold text-gray-900">Faredown Technologies Pvt. Ltd.</p>
          ...
          <div>
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <p className="font-semibold text-gray-900">{booking.guestName || "Guest Name"}</p>
              {booking.address && (
                <p className="text-gray-600 text-sm mt-1">{booking.address}</p>
              )}
              {booking.gstNumber && (
                <p className="text-sm text-gray-600 mt-2">GST No: {booking.gstNumber}</p>
              )}
              <div className="mt-3 space-y-1">
                {booking.phone && (
                  <p className="text-sm text-gray-600 flex items-center">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                ...
                <p className="font-semibold text-lg text-[#003580]">{booking.bookingRef}</p>
                
                <p className="text-sm text-gray-600 mb-1 mt-3">Vehicle</p>
                <p className="font-semibold">{booking.vehicleName}</p>
                <p className="text-sm text-gray-600">{booking.vehicleClass} • {booking.transferType}</p>
              ...
              <tbody>
                <tr className="border-t">
                  ...
                  <td className="py-3 px-4">
                    <div>
                      <p className="font-medium">{booking.transferType} - {booking.vehicleName}</p>
                      <p className="text-sm text-gray-600">{booking.pickupLocation} → {booking.dropoffLocation}</p>
                      <p className="text-sm text-gray-600">{formatDate(booking.pickupDate)} at {booking.pickupTime}</p>
                      {booking.isRoundTrip && (
                        <p className="text-sm text-gray-600">Return: {formatDate(booking.returnDate || "")} at {booking.returnTime || "TBD"}</p>
                      )}
                    </div>
                  </td>
                  <td className="py-3 px-4 text-right">1</td>
                  <td className="py-3 px-4 text-right">₹{baseAmount.toLocaleString()}</td>
                  <td className="py-3 px-4 text-right">₹{baseAmount.toLocaleString()}</td>
                </tr>
              </tbody>
            </table>
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <div className="space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{booking.paymentMethod || "Online Payment"}</span>
                </div>
                {booking.transactionId && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">Transaction ID:</span>
                    <span className="font-medium text-sm">{booking.transactionId}</span>
                  </div>
                )}
                <div className="flex justify-between">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">₹{(taxAmount / 2).toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="border-t pt-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xl font-bold text-[#003580]">₹{totalAmount.toLocaleString()}</span>
                  </div>
                </div>
        ...
        <div className="mb-8">
          ...
          <div className="bg-gray-50 p-4 rounded-lg text-sm text-gray-700">
            <ol className="list-decimal list-inside space-y-1">
              ...
              <li>Free waiting time is included as specified in the booking.</li>
              <li>Driver contact details will be shared 24 hours before pickup.</li>
              <li>Any tolls or parking charges are additional unless specified.</li>
        ...
        <div className="border-t pt-6 text-center text-sm text-gray-500">
          ...
          <p>For support, contact us at support@faredown.com or +91-1234567890</p>
          <p className="mt-2">Invoice generated on {new Date().toLocaleDateString()} | Booking ID: {booking.id}</p>
        </div>
      </div>
    </div>
  );
