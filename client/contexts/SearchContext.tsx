...
export function SearchProvider({ children }: { children: ReactNode }) {
  ...
  const loadFromUrlParams = React.useCallback(
    (urlParams: URLSearchParams) => {
      const newParams: Partial<UnifiedSearchParams> = {};

      // Location/Destination
      if (urlParams.get("destination"))
        newParams.destination = urlParams.get("destination")!;
      if (urlParams.get("destinationCode"))
        newParams.destinationCode = urlParams.get("destinationCode")!;
      if (urlParams.get("destinationName"))
        newParams.destinationName = urlParams.get("destinationName")!;
      ...
      if (urlParams.get("tripType")) {
        newParams.tripType = urlParams.get("tripType") as
          | "one-way"
          | "round-trip"
          | "multi-city";
      }
  ...
  const loadFromBookingData = React.useCallback(
    (booking: any) => {
      if (!booking) return;
      ...
      if (booking.hotel?.location || booking.hotel?.name) {
        ...
        newParams.destinationName =
          booking.hotel.location || booking.hotel.name;
      }
      if (booking.guests)
        newParams.guests = { adults: booking.guests, children: 0 };
      ...
      if (booking.searchParams) {
        if (booking.searchParams.from)
          newParams.destination = booking.searchParams.from;
        if (booking.searchParams.to && !newParams.destination)
          newParams.destination = booking.searchParams.to;
  ...
  const getDisplayData = React.useCallback(() => {
    ...
    return {
      destination:
        searchParams.destinationName || searchParams.destination || "Dubai",
  ...
  const generateUrlParams = React.useCallback(() => {
    ...
    if (searchParams.destinationName)
      params.set("destinationName", searchParams.destinationName);
    if (searchParams.destinationCode)
      params.set("destinationCode", searchParams.destinationCode);
    ...
    if (searchParams.transferType)
      params.set("transferType", searchParams.transferType);
