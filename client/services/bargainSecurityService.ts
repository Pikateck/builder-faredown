...
class BargainSecurityService {
  ...
  private getClientIP(): string {
    ...
    return 'client_' + (localStorage.getItem('client_id') || this.generateClientId());
  }
  ...
  // Check rate limit
  checkRateLimit(type: 'perIP' | 'perSession' | 'perUser', identifier?: string): boolean {
  ...
  sanitizeForLogging(data: any): any {
    ...
    const sensitiveFields = [
      'email', 'phone', 'credit_card', 'password', 'token',
  ...
  setFeatureFlag(flag: keyof typeof this.featureFlags, value: boolean | number): void {
  ...
  loadFeatureFlags(): void {
    for (const flag in this.featureFlags) {
      ...
      if (stored !== null) {
        ...
        this.featureFlags[flag as keyof typeof this.featureFlags] = value as never;
      }
    }
  }
  ...
  getSecurityMetrics(): SecurityMetrics & { 
    ...
    const rateLimitStatus = Array.from(this.rateLimits.entries()).map(([key, limit]) => ({
      key,
      count: limit.count,
