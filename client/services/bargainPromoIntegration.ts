...
export class BargainPromoIntegrationService {
  ...
  async integratePromoWithBargain(
    ...
    if (bargainSuccess) {
      flowExplanation.push(`Step 3: Bargain successful at ₹${bargainedPrice.toLocaleString()}`);
    } else {
      flowExplanation.push(`Step 3: No bargain applied, proceeding with marked-up price`);
    }
      ...
      try {
        ...
        if (promoValidation.valid) {
          ...
          if (proposedFinalPrice >= minimumFinalPrice) {
            ...
            promoApplied = true;
            flowExplanation.push(`Step 4: Promo "${promoCode}" applied = -₹${promoDiscount.toLocaleString()}`);
          } else {
            ...
            promoApplied = true;
            promoAdjusted = true;
            flowExplanation.push(`Step 4: Promo "${promoCode}" adjusted to ₹${promoDiscount.toLocaleString()} (minimum markup protection)`);
          }
        } else {
          flowExplanation.push(`Step 4: Promo "${promoCode}" invalid or expired`);
        }
      } catch (error) {
    ...
    if (finalPrice < netFare) {
      ...
      promoAdjusted = true;
      flowExplanation.push(`Step 5: Final price adjusted to protect net fare = ₹${finalPrice.toLocaleString()}`);
    } else {
      flowExplanation.push(`Step 5: Final price = ₹${finalPrice.toLocaleString()}`);
    }
  ...
  async validatePromoAfterBargain(
    ...
    try {
      ...
      const minimumFinalPrice = netFare * (1 + minimumMarkupPercent / 100);
      const maxAllowedDiscount = Math.max(0, bargainedPrice - minimumFinalPrice);
...
export const bargainPromoIntegrationService = new BargainPromoIntegrationService();
