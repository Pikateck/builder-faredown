...
class EnhancedHotelsService extends EnhancedApiService {
  ...
  private createFallbackHotels(destination?: string, checkIn?: string, checkOut?: string): Hotel[] {
    const destinationName = destination || 'Dubai';
    const checkInDate = checkIn || new Date().toISOString().split('T')[0];
    const checkOutDate = checkOut || new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
