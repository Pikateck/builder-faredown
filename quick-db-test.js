...
async function quickTest() {
  try {
    ...
    const countResult = await pool.query(
      "SELECT module, COUNT(*) as count FROM module_markups GROUP BY module ORDER BY module",
    );
    if (countResult.rows.length === 0) {
      console.log(
        "   ⚠️  No records found (this is normal for new installation)",
      );
    } else {
    ...
    console.log("\n6. Testing UPDATE...");
    await pool.query(
      "UPDATE module_markups SET markup_value = $1, updated_at = NOW() WHERE id = $2",
      [15.0, testId],
    );
