  ...
  try {
    ...
    try {
      ...
      if (health.data?.adapters) {
        console.log('Available adapters:', Object.keys(health.data.adapters).join(', '));
        Object.entries(health.data.adapters || {}).forEach(([name, status]) => {
          console.log(`   - ${name}: ${status.healthy ? '✅ Healthy' : '❌ Unhealthy'}`);
        });
      }
    } catch (e) {
      console.log('⚠️  Health check not available (expected on some endpoints)');
    }
    ...
    if (searchResult.success) {
      ...
      if (supplierCounts.RATEHAWK) {
        ...
        console.log(`   Actually got: ${Object.keys(supplierCounts).join(' + ')}`);
      } else {
        console.log('\n❌ PROBLEM: RateHawk data NOT found in results');
        console.log(`   Only getting: ${Object.keys(supplierCounts).join(', ')}`);
        console.log('\n   Possible causes:');
        console.log('   1. RateHawk not enabled in suppliers table (is_enabled = FALSE)');
        console.log('   2. RateHawk adapter not initialized (missing env vars)');
        console.log('   3. RateHawk adapter encountered an error during search');
      }
