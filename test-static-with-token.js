      ...
      try {
        ...
        if (cities && cities.length > 0) {
          ...
          const dubai = cities.find(c => {
            ...
            return name.includes("dubai") || c.CountryCode === "AE" || c.CityCode === "DXB";
          });
          ...
          if (dubai) {
            ...
            fs.writeFileSync('dubai-cityid.json', JSON.stringify(dubai, null, 2));
            console.log("");
          ...
          // Save full list
          fs.writeFileSync('tbo-cities-full.json', JSON.stringify(cities, null, 2));
          console.log(`  💾 Saved ${cities.length} cities to tbo-cities-full.json`);
        ...
        if (i === payloads.length - 1) {
          console.log(`  ❌ All payloads failed: ${error.response?.status || error.message}`);
        }
      }
    }
    console.log("");
