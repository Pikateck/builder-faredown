  ...
  try {
    ...
    if (tableCheck.rows[0].exists) {
      ...
      const count = await client.query(
        "SELECT COUNT(*) FROM transfers_markups",
      );
      console.log(`📊 Current records: ${count.rows[0].count}`);
    } else {
      console.log(
        "⚠️  transfers_markups table does not exist - need to run migration",
      );
      ...
      if (fs.existsSync(schemaPath)) {
        ...
        console.log(
          `📊 Initial records inserted: ${verifyCount.rows[0].count}`,
        );
      } else {
