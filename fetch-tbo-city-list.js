...
async function fetchCityList() {
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      if (response.data) {
        ...
        if (cities && cities.length > 0) {
          ...
          const dubaiVariations = cities.filter((c) => {
            ...
            return (
              name.toLowerCase().includes("dubai") ||
              c.CityCode === "DXB" ||
              c.CountryCode === "AE"
            );
          });
          ...
          if (dubaiVariations.length > 0) {
            fs.writeFileSync(
              "tbo-dubai-cities.json",
              JSON.stringify(dubaiVariations, null, 2),
            );
      ...
      console.log(
        `  ❌ HTTP ${status}: ${typeof message === "string" ? message.substring(0, 100) : JSON.stringify(message).substring(0, 100)}`,
      );
    }
    console.log("");
  }
  ...
  console.log("");
  console.log(
    "Alternative: Use hardcoded major city mapping based on TBO documentation",
  );
}
