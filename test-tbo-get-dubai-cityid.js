...
const CONFIG = {
  ...
  baseUrl: "https://hotelbooking.travelboutiqueonline.com/HotelAPI_V10/HotelService.svc/rest/",
...
async function getDestinationSearchStaticData(tokenId) {
  ...
  try {
    ...
    // Save full response
    fs.writeFileSync("tbo-destination-static-data-full.json", JSON.stringify({
      request: request,
    ...
    if (Array.isArray(destinations) && destinations.length > 0) {
      ...
      if (dubai) {
        ...
        // Save Dubai data
        fs.writeFileSync("dubai-destination-data.json", JSON.stringify({
        ...
        console.log("Sample destinations:", destinations.slice(0, 5).map(d => 
          d.CityName || d.Name || d.DestinationName
        ));
        return null;
      }
    } else {
...
async function getHotelResult(tokenId, cityId) {
  ...
  try {
    ...
    if (response.data?.HotelResults?.length > 0) {
      console.log("\n✅ SUCCESS! Found", response.data.HotelResults.length, "hotels");
      console.log("\nSample Hotels:");
    ...
    // Save full response
    fs.writeFileSync("tbo-hotel-search-success.json", JSON.stringify({
      request: request,
      response: response.data,
      timestamp: new Date().toISOString()
    }, null, 2));
    ...
    // Save error response
    fs.writeFileSync("tbo-hotel-search-error.json", JSON.stringify({
      request: request,
...
async function main() {
  console.log("\n╔" + "═".repeat(78) + "╗");
  console.log("║" + " ".repeat(15) + "TBO COMPLETE FLOW: AUTH → STATIC → SEARCH" + " ".repeat(22) + "║");
  ...
  const hotelResults = await getHotelResult(tokenId, destinationData.destinationId);
  ...
  if (hotelResults && hotelResults.HotelResults?.length > 0) {
    ...
    console.log("   Dubai Data:", JSON.stringify(destinationData.dubai, null, 2));
    ...
    console.log("\n📁 FILES CREATED:");
    console.log("  - tbo-destination-static-data-full.json (static data response)");
