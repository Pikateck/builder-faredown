...
async function checkCities() {
  ...
  try {
    ...
    `);
    console.log('Cities directly in Europe region:', directCities.rows[0].count);
    ...
    if (parseInt(countriesInEurope.rows[0].count) > 0) {
      ...
      `);
      console.log('Cities via countries in Europe:', citiesViaCountries.rows[0].count);
    }
    ...
    sampleData.rows.forEach(row => {
      console.log(`- ${row.city_name} in ${row.country_name || 'Unknown'} (city region_id: ${row.region_id}, country region_id: ${row.country_region_id})`);
    });
