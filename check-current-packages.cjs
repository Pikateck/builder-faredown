...
async function checkPackages() {
  try {
    ...
    // Get sample packages
    const samplePackages = await pool.query('SELECT title, city_id, country_id, region_id, status FROM packages LIMIT 10');
    console.log('\n📋 Sample packages:');
    ...
    destinations.rows.forEach(dest => {
      console.log(`- ${dest.city_name || 'N/A'}, ${dest.country_name || 'N/A'} (${dest.region_name || 'N/A'}): ${dest.package_count} packages`);
    });
