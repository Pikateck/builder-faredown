...
async function fixForeignKeyDataTypes() {
  ...
  try {
    ...
    if (middleEastRegion.rows.length > 0 && dubaiCity.rows.length > 0) {
      ...
      const countryId = uaeCountry.rows.length > 0 ? uaeCountry.rows[0].id : null;
      ...
      const existingCategories = dubaiPackages.rows.map(row => row.package_category);
      
      for (const category of requiredCategories) {
        if (!existingCategories.includes(category)) {
          const basePrice = category === 'luxury' ? 179998 : (category === 'standard' ? 89999 : 49999);
          const duration = category === 'luxury' ? 7 : (category === 'standard' ? 5 : 4);
          ...
          `, [
            ...
            'active',
            category === 'luxury',
            JSON.stringify(category === 'luxury' 
              ? ["5-star hotel accommodation", "Private airport transfers", "Personal guide", "Premium experiences", "Burj Khalifa At The Top"]
              : category === 'standard' 
              ? ["4-star hotel accommodation", "Group transfers", "Guided tours", "Dubai Mall visits", "Desert safari"]
              : ["3-star hotel accommodation", "Shared transfers", "Basic sightseeing", "Dubai Marina walk", "Local experiences"]
            ),
            JSON.stringify(["International flights", "Personal expenses", "Travel insurance", "Optional activities", "Meals not specified"])
          ]);
    ...
    for (const pkg of packagesWithoutDepartures.rows) {
      for (const depDate of departureDates) {
        ...
        `, [
          ...
          'INR', 10, new Date(new Date(depDate).getTime() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
          true, true, 'Guaranteed departure with minimum 2 passengers'
        ]);
      }
    ...
    verification.rows.forEach(row => {
      ...
      console.log(`  Location: ${row.city_name}, ${row.country_name} (${row.region_name})`);
      console.log(`  Departures: ${row.departure_count}`);
      console.log('');
    });
    
    console.log('🎉 Foreign key data type fix completed successfully!');
    console.log('🎯 Now Dubai packages should filter correctly by destination and date!');
