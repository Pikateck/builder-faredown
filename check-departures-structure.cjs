...
async function checkDeparturesStructure() {
  try {
    ...
    const count = await pool.query('SELECT COUNT(*) as count FROM package_departures');
    console.log(`\nExisting departures: ${count.rows[0].count}`);
    
    if (count.rows[0].count > 0) {
      const sample = await pool.query('SELECT * FROM package_departures LIMIT 3');
      console.log('\nSample departures:');
      sample.rows.forEach((row, i) => {
        console.log(`${i + 1}. Package ID: ${row.package_id}, Date: ${row.departure_date}, Price: ${row.price_per_person}`);
      });
    }
