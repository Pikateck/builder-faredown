...
async function debugRecentSearches() {
  try {
    ...
    allResults.rows.forEach((row, i) => {
      console.log(`${i + 1}. ID: ${row.id}, User: ${row.user_id || 'null'}, Device: ${row.device_id || 'null'}`);
      console.log(`   Module: ${row.module}, Hash: ${row.query_hash.substring(0, 16)}...`);
    ...
    deviceResults.rows.forEach((row, i) => {
      console.log(`${i + 1}. ${row.query.from.name} → ${row.query.to.name} (${row.query.tripType})`);
    });
