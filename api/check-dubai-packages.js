...
async function checkDubaiPackages() {
  try {
    ...
    console.log(
      '\n🏙️ Packages with "Dubai" in title:',
      dubaiResult.rows.length,
    );

    dubaiResult.rows.forEach((pkg, index) => {
      console.log(
        `  ${index + 1}. ID: ${pkg.id} | ${pkg.title} | ${pkg.base_price_pp} ${pkg.currency || "USD"} | Status: ${pkg.status}`,
      );
    });
    ...
    const apiResult = await pool.query(apiFilterQuery, [
      `%${destinationName}%`,
    ]);
    ...
    apiResult.rows.forEach((pkg, index) => {
      console.log(
        `  ${index + 1}. ${pkg.title} | ${pkg.base_price_pp} ${pkg.currency || "USD"}`,
      );
    });
