...
describe('PricingEngine', () => {
  ...
  describe('Parameter Validation', () => {
    ...
    test('should validate module values', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('module must be one of: air, hotel, sightseeing, transfer');
    });
  ...
  describe('Complete Quote Calculation', () => {
    test('should calculate quote with percentage markup and discount', async () => {
      ...
      expect(result.tax).toBe(125.40);
      expect(result.totalFare).toBe(1170.40);
    ...
    test('should apply max discount limit', async () => {
      ...
      const mockPromo = {
        type: 'percent',
        value: 50.00, // 50% would be 500, but max_discount is 100
        max_discount: 100.00
      };
  ...
  describe('Error Handling', () => {
    test('should handle database errors gracefully', async () => {
      ...
      await expect(pricingEngine.getApplicableMarkupRule({}))
        .rejects
