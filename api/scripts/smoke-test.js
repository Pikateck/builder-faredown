...
class SmokeTestRunner {
  ...
  async makeRequest(path, data, method = 'POST') {
    ...
    return new Promise((resolve, reject) => {
      const options = {
        ...
        port: CONFIG.host.split(':')[1] || (CONFIG.host.includes('https') ? 443 : 80),
        path: path,
        method: method,
        headers: {
  ...
  async testSessionStart(testCase, caseName) {
    ...
    try {
      const response = await this.makeRequest('/api/bargain/v1/session/start', testCase);
      ...
      if (response.duration > CONFIG.maxLatency) {
        this.results.errors.push(`❌ ${caseName}: Latency ${response.duration.toFixed(2)}ms > ${CONFIG.maxLatency}ms`);
      } else {
      ...
      console.log(`✅ ${caseName}: Session ${data.session_id}, offer ${data.initial_offer.price}, floor ${data.min_floor}`);
  ...
  async testOfferFlow(sessionData, caseName) {
    ...
    try {
      ...
      if (data.decision !== 'counter' && data.decision !== 'reject') {
        console.log(`⚠️ Expected counter/reject for low offer, got: ${data.decision}`);
      } else {
      ...
      if (offerResponse.duration > CONFIG.maxLatency) {
        this.results.errors.push(`❌ ${caseName} offer: Latency ${offerResponse.duration.toFixed(2)}ms > ${CONFIG.maxLatency}ms`);
      }
  ...
  async testNeverLossValidation() {
    ...
    try {
      ...
      if (response.data.decision === 'accept') {
        throw new Error(`Never-loss violation: System accepted offer ${veryLowOffer} below floor ${session.min_floor}`);
      }
      
      console.log(`✅ Never-loss enforced: offer ${veryLowOffer} correctly rejected/countered`);
      this.results.passed++;
  ...
  async testCapsuleVerification() {
    ...
    try {
      ...
      console.log(`✅ Capsule structure valid, signature: ${capsule.signature.substring(0, 20)}...`);
      this.results.passed++;
  ...
  generateReport() {
    console.log(`\n📊 SMOKE TEST REPORT`);
    console.log(`==================`);
    console.log(`Duration: ${((Date.now() - this.results.startTime) / 1000).toFixed(1)}s`);
