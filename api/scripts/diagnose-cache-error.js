...
async function diagnose() {
  ...
  try {
    ...
    for (const table of tables) {
      ...
      if (result.rows[0].exists) {
        const countResult = await db.query(`SELECT COUNT(*) as count FROM public.${table}`);
  ...
  try {
    ...
    if (adapter) {
      ...
      try {
        ...
        if (token) {
          log('green', `   ✅ TBO Token obtained: ${token.substring(0, 20)}...`);
        } else {
  ...
  try {
    ...
    if (hotelCacheService) {
      ...
      if (hash && hash.length === 64) {
        log('green', `   ✅ Hash generation working: ${hash.substring(0, 16)}...`);
      } else {
  ...
  if (issues.length === 0) {
    ...
    if (issues.some(i => i.includes('environment variable'))) {
      log('blue', '   • Verify TBO credentials are set in Render environment variables');
      log('blue', '   • Check: TBO_HOTEL_CLIENT_ID, TBO_HOTEL_USER_ID, TBO_HOTEL_PASSWORD');
    }
    ...
    if (issues.some(i => i.includes('Cache table'))) {
      log('blue', '   • Re-run migration: psql $DATABASE_URL < api/database/migrations/20250205_hotel_cache_layer.sql');
    }
