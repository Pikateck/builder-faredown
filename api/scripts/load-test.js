...
export default function () {
  ...
  const sessionRes = http.post(`${BASE_URL}/api/bargain/v1/session/start`, sessionPayload, { headers });
  ...
  const offerRes = http.post(`${BASE_URL}/api/bargain/v1/session/offer`, offerPayload, { headers });
  ...
  if (Math.random() < 0.3) {
    ...
    const acceptRes = http.post(`${BASE_URL}/api/bargain/v1/session/accept`, acceptPayload, { headers });
    ...
    const acceptCheck = check(acceptRes, {
      ...
      'accept has expected response': (r) => {
        try {
          const body = JSON.parse(r.body);
          return r.status === 409 ? body.error !== undefined : body.final_price !== undefined;
        } catch (e) {
          return false;
        }
      }
...
export function handleSummary(data) {
  ...
  console.log(`HTTP p95 Latency: ${summary.http_req_duration_p95.toFixed(2)}ms`);
  console.log(`HTTP Error Rate: ${(summary.http_req_failed_rate * 100).toFixed(3)}%`);
  ...
  console.log(`ERROR RATE PASSED (< 0.5%): ${summary.passed_error_rate ? '✅' : '❌'}`);
