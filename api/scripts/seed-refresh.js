...
class SeedRefreshRunner {
  ...
  async generateSyntheticSessions(count = 200) {
    ...
    const activities = ['city-tour', 'museum-visit', 'boat-ride', 'cultural-show'];
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (productType === 'flight') {
        ...
        displayedPrice = classType === 'business' ? 400 + Math.random() * 600 : 150 + Math.random() * 300;
        ...
        displayedPrice = roomType === 'deluxe' ? 200 + Math.random() * 400 : 80 + Math.random() * 200;
      ...
      `, [
        sessionId, userId, userTier, deviceType,
        cpoKey, displayedPrice, trueCost,
      ...
      for (let j = 0; j < eventCount; j++) {
        ...
        const eventType = j === eventCount - 1 ? 
          (finalOutcome === 'accepted' ? 'accept' : 'offer') : 'offer';
        
        const userOffer = Math.round(trueCost * (1.1 + Math.random() * 0.4)); // 110-150% of cost
        const counterPrice = eventType === 'accept' ? userOffer : 
        ...
        `, [
          sessionId, eventType, userOffer, counterPrice,
      ...
      await this.pg.query(`
        ...
        WHERE session_id = $3
      `, [finalOutcome, new Date(timestamp.getTime() + eventCount * 2 * 60000), sessionId]);
  ...
  async seedPromoCodes() {
    ...
    const promos = [
      { code: 'SAVE10', type: 'percentage', value: 10, description: '10% off all bookings' },
      { code: 'SAVE20', type: 'percentage', value: 20, description: '20% off hotels' },
      { code: 'FLAT50', type: 'flat', value: 50, description: 'Flat $50 off flights' },
      { code: 'WELCOME', type: 'percentage', value: 15, description: 'Welcome bonus' },
      { code: 'GOLD25', type: 'percentage', value: 25, description: 'Gold member exclusive' }
    ];
