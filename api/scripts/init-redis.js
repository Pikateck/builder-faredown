...
async function initRedis() {
  ...
  try {
    ...
    const policyResult = await pool.query('SELECT * FROM ai.policies WHERE version = $1', ['v1']);
    ...
    const suppliersResult = await pool.query('SELECT * FROM ai.suppliers WHERE active = true');
    ...
    const modelsResult = await pool.query('SELECT * FROM ai.model_registry WHERE active = true');
    ...
    const abTestsResult = await pool.query('SELECT * FROM ai.ab_tests WHERE active = true');
    ...
    for (const [canonicalKey, snapshots] of Object.entries(snapshotsByProduct)) {
    ...
    const userProfilesResult = await pool.query('SELECT * FROM ai.user_profiles LIMIT 10');
    ...
    console.log('🎯 AI Bargaining Platform cache is ready for <300ms responses');
