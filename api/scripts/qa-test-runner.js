...
class QATestRunner {
  ...
  async testHotelDoubleCounter() {
    ...
    try {
      // Start session
      const sessionResponse = await this.makeRequest('/api/bargain/v1/session/start', {
      ...
      if (sessionResponse.status !== 200) {
        this.logTest('Hotel Session Start', 'FAIL', `Status ${sessionResponse.status}`);
        return;
      }

      const session = sessionResponse.data;
      this.logTest('Hotel Session Start', 'PASS', `Session ${session.session_id}`);

      // First offer - slightly below initial
      const firstOffer = Math.round(session.initial_offer.price * 0.85);
      const firstOfferResponse = await this.makeRequest('/api/bargain/v1/session/offer', {
        session_id: session.session_id,
        user_offer: firstOffer,
        signals: { time_on_page: 45, scroll_depth: 80, device_type: 'desktop' }
      });

      if (firstOfferResponse.status !== 200) {
        this.logTest('Hotel First Offer', 'FAIL', `Status ${firstOfferResponse.status}`);
        return;
      }
      ...
      if (firstResult.decision === 'counter' && firstResult.counter_offer) {
        ...
        const acceptResponse = await this.makeRequest('/api/bargain/v1/session/accept', {
          session_id: session.session_id
        });
        
        if (acceptResponse.status === 200) {
          this.logTest('Hotel First Counter Accept', 'PASS', 
            `Accepted counter ${firstResult.counter_offer}`);
        } else {
          this.logTest('Hotel First Counter Accept', 'FAIL', 
            `Status ${acceptResponse.status}`);
          return;
        }

        // Start a new session for second bargain
        const secondSessionResponse = await this.makeRequest('/api/bargain/v1/session/start', {
          user: { id: 'qa_hotel_user', tier: 'GOLD', device_type: 'desktop' },
          productCPO: {
            type: 'hotel',
            supplier: 'hotelbeds',
        ...
        if (secondSessionResponse.status === 200) {
          ...
          const secondOfferResponse = await this.makeRequest('/api/bargain/v1/session/offer', {
          ...
          if (secondOfferResponse.status === 200) {
            ...
            if (secondResult.decision === 'counter' && secondResult.counter_offer) {
              // Accept second counter
              const accept2Response = await this.makeRequest('/api/bargain/v1/session/accept', {
                session_id: session2.session_id
              });
              
              if (accept2Response.status === 200) {
                this.logTest('Hotel Double Counter Flow', 'PASS', 
                  `Two successful counter-accepts: ${firstResult.counter_offer} & ${secondResult.counter_offer}`);
              } else {
                this.logTest('Hotel Double Counter Flow', 'FAIL', 
                  'Second accept failed');
              }
            } else {
              this.logTest('Hotel Double Counter Flow', 'PARTIAL', 
  ...
  async testFlightBelowFloorFlow() {
    ...
    try {
      ...
      const sessionResponse = await this.makeRequest('/api/bargain/v1/session/start', {
        user: { id: 'qa_flight_user', tier: 'standard', device_type: 'mobile' },
        productCPO: {
          type: 'flight',
          supplier: 'amadeus',
      ...
      if (sessionResponse.status !== 200) {
        this.logTest('Flight Session Start', 'FAIL', `Status ${sessionResponse.status}`);
        return;
      }

      const session = sessionResponse.data;
      this.logTest('Flight Session Start', 'PASS', `Floor: ${session.min_floor}`);

      // Offer below floor (should get counter)
      const belowFloorOffer = Math.round(session.min_floor * 0.8);
      const offerResponse = await this.makeRequest('/api/bargain/v1/session/offer', {
        session_id: session.session_id,
        user_offer: belowFloorOffer
      });

      if (offerResponse.status !== 200) {
        this.logTest('Flight Below Floor Offer', 'FAIL', `Status ${offerResponse.status}`);
        return;
      }
      ...
      if (result.decision === 'counter' || result.decision === 'reject') {
        if (result.counter_offer && result.counter_offer >= session.min_floor) {
          this.logTest('Flight Floor Protection', 'PASS', 
          ...
          if (acceptResponse.status === 200) {
            this.logTest('Flight Counter Accept', 'PASS', 
              `Accepted counter ${result.counter_offer}`);
          } else {
            this.logTest('Flight Counter Accept', 'FAIL', 
              `Status ${acceptResponse.status}`);
          }
        } else {
          this.logTest('Flight Floor Protection', 'FAIL', 
  ...
  async testErrorHandling() {
    ...
    try {
      ...
      const staleRateResponse = await this.makeRequest('/api/bargain/v1/session/start', {
      ...
      if (staleRateResponse.status === 409 || 
          (staleRateResponse.data && staleRateResponse.data.error_code === 'RATE_STALE')) {
        this.logTest('RATE_STALE Error', 'PASS', 'Properly returned stale rate error');
      } else {
        this.logTest('RATE_STALE Error', 'FAIL', 'Did not trigger RATE_STALE error');
      }

      // Test inventory change during accept
      const inventoryTestSession = await this.makeRequest('/api/bargain/v1/session/start', {
      ...
      if (inventoryTestSession.status === 200) {
        ...
        const inventoryAcceptResponse = await this.makeRequest('/api/bargain/v1/session/accept', {
          session_id: session.session_id,
          force_error: 'INVENTORY_CHANGED'  // Test flag
        });

        if (inventoryAcceptResponse.status === 409) {
          this.logTest('INVENTORY_CHANGED Error', 'PASS', 'Properly returned inventory error');
        } else {
          this.logTest('INVENTORY_CHANGED Error', 'FAIL', 'Did not trigger INVENTORY_CHANGED error');
        }
      }

      // Test policy blocked (blackout dates)
      const blackoutResponse = await this.makeRequest('/api/bargain/v1/session/start', {
      ...
      if (blackoutResponse.status === 403 || 
          (blackoutResponse.data && blackoutResponse.data.error_code === 'POLICY_BLOCKED')) {
        this.logTest('POLICY_BLOCKED Error', 'PASS', 'Properly blocked blackout dates');
      } else {
        this.logTest('POLICY_BLOCKED Error', 'SKIP', 'No blackout policy configured');
      }
  ...
  async testPromosAndTiers() {
    ...
    try {
      ...
      const percentPromoResponse = await this.makeRequest('/api/bargain/v1/session/start', {
        user: { id: 'qa_promo_user', tier: 'standard', device_type: 'desktop' },
        productCPO: {
          type: 'hotel',
          supplier: 'hotelbeds',
      ...
      if (percentPromoResponse.status === 200) {
        ...
        const hasPromoData = session.initial_offer.explanation && 
                            session.initial_offer.explanation.includes('promo');
        
        this.logTest('Percentage Promo', hasPromoData ? 'PASS' : 'FAIL', 
          `Promo applied: ${hasPromoData}`);
      } else {
        this.logTest('Percentage Promo', 'FAIL', `Status ${percentPromoResponse.status}`);
      }

      // Test flat amount promo
      const flatPromoResponse = await this.makeRequest('/api/bargain/v1/session/start', {
      ...
      if (flatPromoResponse.status === 200) {
        ...
        this.logTest('Flat Amount Promo', 'FAIL', `Status ${flatPromoResponse.status}`);
      }

      // Test PLATINUM tier bonus
      const platinumResponse = await this.makeRequest('/api/bargain/v1/session/start', {
        user: { id: 'qa_platinum_user', tier: 'PLATINUM', device_type: 'desktop' },
        productCPO: {
          type: 'hotel',
          supplier: 'hotelbeds',
      ...
      if (platinumResponse.status === 200) {
        ...
        const tierOfferResponse = await this.makeRequest('/api/bargain/v1/session/offer', {
          session_id: session.session_id,
        ...
        if (tierOfferResponse.status === 200) {
          ...
          const hasTierNudge = result.explain && 
                              (result.explain.includes('PLATINUM') || 
                               result.explain.includes('tier') ||
                               result.explain.includes('valued'));
          
          this.logTest('PLATINUM Tier Bonus', hasTierNudge ? 'PASS' : 'PARTIAL', 
  ...
  async testMultiSupplierArbitration() {
    ...
    try {
      ...
      const multiSupplierResponse = await this.makeRequest('/api/bargain/v1/session/start', {
      ...
      if (multiSupplierResponse.status === 200) {
        ...
        if (offerResponse.status === 200 && offerResponse.data.decision === 'accept') {
          const acceptResponse = await this.makeRequest('/api/bargain/v1/session/accept', {
            session_id: session.session_id
          });
          ...
          if (acceptResponse.status === 200) {
            ...
            const hasSupplierInfo = booking.payment_payload && 
                                   booking.payment_payload.supplier_info;
            
            this.logTest('Multi-Supplier Arbitration', hasSupplierInfo ? 'PASS' : 'PARTIAL', 
              `Supplier selection: ${hasSupplierInfo ? 'included' : 'not included'}`);
          } else if (acceptResponse.status === 409) {
            // Lock failure - test fallback
            this.logTest('Multi-Supplier Fallback', 'PASS', 
              'Lock conflict handled - fallback triggered');
          }
        }
      } else {
        this.logTest('Multi-Supplier Setup', 'SKIP', 'Multi-supplier test data not available');
      }
