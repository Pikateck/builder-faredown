...
async function testCacheMiss() {
  ...
  try {
    ...
    log(
      "blue",
      `Sample hotel: ${hotel.name} (${hotel.starRating}⭐) - ₹${hotel.price?.offered || hotel.price}`,
    );
...
async function testCacheHit() {
  ...
  try {
    ...
    if (response.body.cacheHit) {
      ...
      log(
        "green",
        `Cached at: ${new Date(response.body.cachedAt).toLocaleTimeString()}`,
      );
      log(
        "green",
        `Expires at: ${new Date(response.body.ttlExpiresAt).toLocaleTimeString()}`,
      );
      ...
      if (response.duration > 500) {
        log(
          "yellow",
          `⚠️  Cache hit took ${response.duration}ms (expected <200ms)`,
        );
      } else {
...
async function testRoomDetails(hotelId) {
  ...
  try {
    ...
    if (response.body.rooms && response.body.rooms.length > 0) {
      ...
      log(
        "blue",
        `Sample room: ${room.roomTypeName} - ₹${room.price?.base || 0}/night`,
      );
    }
...
async function testCacheStats() {
  ...
  try {
    ...
    log(
      "blue",
      `Avg hotels per search: ${parseFloat(stats.avg_hotels_per_search || 0).toFixed(1)}`,
    );
...
async function runAllTests() {
  ...
  log(
    "yellow",
    `📍 Search: ${searchParams.destination} (${searchParams.checkIn} to ${searchParams.checkOut})`,
  );
  ...
  if (
    results.test1 &&
    results.test1.hotels &&
    results.test1.hotels.length > 0
  ) {
