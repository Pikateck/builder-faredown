...
async function runTransferMarkupMigration() {
  ...
  try {
    ...
    console.log('📋 Result:', result[result.length - 1]?.rows || 'Migration completed');
    ...
    if (tableCheck.rows.length > 0) {
      ...
      const dataCheck = await client.query('SELECT COUNT(*) FROM transfer_markups');
      console.log(`📊 Transfer markups in database: ${dataCheck.rows[0].count}`);
    } else {
