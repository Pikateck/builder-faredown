...
async function testTransfersBargainFlow() {
  ...
  try {
    ...
    const startResponse = await makeRequest('/session/start', 'POST', startPayload);
    ...
    const firstOfferResponse = await makeRequest('/session/offer', 'POST', firstOffer);
    console.log(`   Status: ${firstOfferResponse.status}`);
    console.log(`   AI Decision: ${firstOfferResponse.data.aiResponse?.decision}`);
    console.log(`   AI Message: ${firstOfferResponse.data.aiResponse?.message}`);
    ...
    const secondOfferResponse = await makeRequest('/session/offer', 'POST', secondOffer);
    console.log(`   Status: ${secondOfferResponse.status}`);
    console.log(`   AI Decision: ${secondOfferResponse.data.aiResponse?.decision}`);
    console.log(`   AI Counter Price: ₹${secondOfferResponse.data.aiResponse?.counterPrice}`);
    console.log(`   AI Message: ${secondOfferResponse.data.aiResponse?.message}`);
    console.log(`   Savings: ₹${secondOfferResponse.data.aiResponse?.savings}`);

    // Test 5: Accept Counter Offer (if there is one)
    if (secondOfferResponse.data.aiResponse?.decision === 'counter' || 
        secondOfferResponse.data.aiResponse?.decision === 'accept') {
      console.log('\n5️⃣ Accepting the bargain...');
      const acceptPayload = { sessionId: sessionId };
      
      const acceptResponse = await makeRequest('/session/accept', 'POST', acceptPayload);
      console.log(`   Status: ${acceptResponse.status}`);
      console.log(`   Booking Reference: ${acceptResponse.data.bookingPayload?.bookingReference}`);
      console.log(`   Final Price: ₹${acceptResponse.data.bookingPayload?.finalPrice}`);
      console.log(`   Savings: ₹${acceptResponse.data.bookingPayload?.savings}`);
    }
