...
class BackgroundJobsService {
  ...
  async runHourlyJobs() {
    ...
    try {
      ...
      this.updateJobStatus(jobId, 'completed', 'Hourly jobs completed successfully');
  ...
  async runNightlyJobs() {
    ...
    try {
      ...
      this.updateJobStatus(jobId, 'completed', 'Nightly jobs completed successfully');
      ...
      for (const view of views) {
        ...
        try {
          ...
          if (error.message.includes('concurrently')) {
            this.logger.warn(`Concurrent refresh failed for ${view}, trying non-concurrent...`);
            await this.pool.query(`REFRESH MATERIALIZED VIEW ${view}`);
            const duration = Date.now() - startTime;
            this.logger.info(`Refreshed ${view} (non-concurrent) in ${duration}ms`);
          } else {
            throw error;
          }
        }
    ...
    try {
      ...
      for (const product of result.rows) {
        try {
          ...
          this.logger.warn(`Failed to warm cache for ${product.canonical_key}:`, error);
        }
      }
    ...
    try {
      ...
      for (const row of result.rows) {
        try {
          ...
          this.logger.warn(`Failed to update features for ${row.canonical_key}:`, error);
        }
      }
  ...
  async refreshHotSupplierSnapshots() {
    try {
      ...
      this.logger.info(`Refreshing snapshots for ${result.rows.length} hot products`);

      // Trigger supplier adapter refresh for these products
      for (const product of result.rows) {
        try {
          await supplierAdapterManager.refreshSupplierSnapshots(product.product_type, 1);
        } catch (error) {
          this.logger.warn(`Failed to refresh snapshot for ${product.canonical_key}:`, error);
        }
      }
  ...
  async updateUserProfiles() {
    try {
      ...
      for (const userStat of result.rows) {
        ...
        const style = userStat.accept_rate > 0.6 ? 'generous' : 
                     userStat.accept_rate > 0.3 ? 'persistent' : 'cautious';
  ...
  async runHealthChecks() {
    try {
      ...
      const overallHealth = {
        ...
        suppliers: healthChecks[1]?.every(s => s.status === 'healthy') || false,
      ...
      const healthyComponents = Object.values(overallHealth).filter(Boolean).length;
