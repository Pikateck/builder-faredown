...
class HotelbedsAdapter extends BaseSupplierAdapter {
  constructor(config = {}) {
    super('HOTELBEDS', {
      baseUrl: process.env.HOTELBEDS_BOOKING_API || 'https://api.test.hotelbeds.com/hotel-api/1.0',
      contentUrl: process.env.HOTELBEDS_CONTENT_API || 'https://api.test.hotelbeds.com/hotel-content-api/1.0',
  ...
  async searchHotels(searchParams) {
    return await this.executeWithRetry(async () => {
      ...
      const occupancies = rooms.map(room => ({
        ...
        ...(room.childAges && { paxes: room.childAges.map((age, index) => ({ 
          type: 'CH', 
          age: age, 
          roomId: 1 
        })) })
      ...
      const normalizedHotels = hotels.map(hotel => this.transformHotelbedsHotel(hotel, searchParams));
      
      // Store in repository and create snapshots
      await this.storeProductsAndSnapshots(normalizedHotels, 'hotel');
      
      this.logger.info(`Retrieved ${normalizedHotels.length} hotel offers from Hotelbeds`);
  ...
  async searchSightseeing(searchParams) {
    return await this.executeWithRetry(async () => {
      ...
      const response = await this.activitiesClient.post('/activities/search', searchRequest);
      ...
      this.logger.info(`Retrieved ${normalizedActivities.length} activity offers from Hotelbeds`);
  ...
  async getSightseeingDetails(activityCode) {
    return await this.executeWithRetry(async () => {
      const response = await this.activitiesClient.get(`/activities/${activityCode}`, {
        params: {
          language: 'en'
        }
      });
  ...
  async bookSightseeing(bookingData) {
    return await this.executeWithRetry(async () => {
      ...
      const response = await this.activitiesClient.post('/bookings', bookingRequest);
  ...
  transformHotelbedsHotel(hotel, searchParams) {
    try {
      ...
      return {
        ...
        starRating: hotel.categoryCode ? parseInt(hotel.categoryCode.charAt(0)) : 0,
  ...
  transformHotelbedsActivity(activity, searchParams) {
    try {
      ...
      return {
        ...
        duration: modality.duration?.value ? `${modality.duration.value}${modality.duration.metric}` : '4H',
        ...
        exclusions: [],
        pickupPoint: modality.amountsFrom?.[0]?.paxType === 'ADULT' ? 'Hotel Pickup Available' : 'Meet at venue',
        ...
        policyFlags: {
          ...
          cancellationPolicy: modality.operation?.cancellationPolicy || 'Standard',
  ...
  async getDestinations(countryCode = null) {
    return await this.executeWithRetry(async () => {
      ...
      const response = await this.contentClient.get('/locations/destinations', { params });
