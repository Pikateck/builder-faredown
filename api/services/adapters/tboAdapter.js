...
class TBOAdapter extends BaseSupplierAdapter {
    ...
    return await this.executeWithRetry(async () => {
      ...
      for (const p of paths) {
        try {
          const egress = await this._getEgressIp();
          const url = `${this.config.hotelAuthBase}${p}`;
          this.logger.info(`TBO Hotel Auth attempt -> ${url}`);
  ...
  async bookHotel(params) {
    ...
    try {
      ...
      const res = await this.executeWithRetry(() =>
        this.hotelBookingClient.post("/Book", payload),
      );
      if (
        res.data?.Status === 1 ||
        res.data?.BookingId ||
  ...
  async generateHotelVoucher(params) {
    ...
    try {
      ...
      const res = await this.executeWithRetry(
        () => this.hotelBookingClient.post("/GenerateVoucher", payload),
        5,
      );
  ...
  async getHotelBookingDetails(params) {
    ...
    try {
      ...
      const res = await this.executeWithRetry(() =>
        this.hotelBookingClient.post("/GetBookingDetail", payload),
      );
  ...
  async cancelHotelBooking(params) {
    ...
    try {
      ...
      const res = await this.executeWithRetry(() =>
        this.hotelBookingClient.post("/SendChangeRequest", payload),
      );
      if (res.data?.Status === 1) {
        return res.data;
      }
  ...
  async getHotelInfo(params) {
    ...
    try {
      ...
      const res = await this.executeWithRetry(() =>
        this.hotelSearchClient.post("/HotelInfo", payload),
      );
  ...
  async getHotelRoom(params) {
    ...
    try {
      ...
      const res = await this.executeWithRetry(() =>
        this.hotelSearchClient.post("/HotelRoom", payload),
      );
      if (res.data?.Status === 1) return res.data;
      throw mapFromResponse(res);
    } catch (e) {
