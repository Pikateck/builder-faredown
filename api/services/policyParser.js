...
class PolicyParser {
  ...
  async getParsedPolicy() {
    ...
    if (this.cachedPolicy && this.lastCacheTime && (now - this.lastCacheTime) < this.cacheValidityMs) {
  ...
  async parseAndCachePolicy() {
    ...
    try {
      ...
      this.logger.info(`Policy parsed and cached: ${policyRow.version} (${parseTime}ms)`);
    ...
    const optimized = {
      ...
      price_rules: {
        flight: {
          ...
          max_discount_pct: yamlPolicy.price_rules?.flight?.max_discount_pct || 0.15,
        ...
        hotel: {
          ...
          max_discount_pct: yamlPolicy.price_rules?.hotel?.max_discount_pct || 0.20,
          hold_minutes: yamlPolicy.price_rules?.hotel?.hold_minutes || 15,
          allow_perks: yamlPolicy.price_rules?.hotel?.allow_perks !== false,
          allowed_perks: yamlPolicy.price_rules?.hotel?.allowed_perks || ['Late checkout', 'Free breakfast']
        },
        sightseeing: {
          min_margin_usd: yamlPolicy.price_rules?.sightseeing?.min_margin_usd || 3.0,
          max_discount_pct: yamlPolicy.price_rules?.sightseeing?.max_discount_pct || 0.25,
          hold_minutes: yamlPolicy.price_rules?.sightseeing?.hold_minutes || 5,
          allow_perks: yamlPolicy.price_rules?.sightseeing?.allow_perks !== false,
          allowed_perks: yamlPolicy.price_rules?.sightseeing?.allowed_perks || ['Skip the line', 'Free guide']
        }
      },
      
      // Indexed by supplier code for O(1) lookup
      supplier_overrides: this.indexSupplierOverrides(yamlPolicy.supplier_overrides || {}),
      
      // Promo rules for stacking validation
      promo_rules: {
        stacking: {
          max_total_discount_pct: yamlPolicy.promo_rules?.stacking?.max_total_discount_pct || 0.25
        },
        eligibility: {
          loyalty_tier_boost: yamlPolicy.promo_rules?.eligibility?.loyalty_tier_boost || {
            GOLD: 1.05,
            PLATINUM: 1.08
          }
        }
      ...
      guardrails: {
        abort_if_inventory_stale_minutes: yamlPolicy.guardrails?.abort_if_inventory_stale_minutes || 5,
        abort_if_latency_ms_over: yamlPolicy.guardrails?.abort_if_latency_ms_over || 280
      },
  ...
  validatePolicyStructure(policy) {
    ...
    if (policy.global) {
      ...
      if (policy.global.response_budget_ms < 100 || policy.global.response_budget_ms > 1000) {
    ...
    if (policy.price_rules) {
      for (const [productType, rules] of Object.entries(policy.price_rules)) {
        ...
        if (rules.max_discount_pct < 0 || rules.max_discount_pct > 1) {
          errors.push(`${productType}: max_discount_pct must be between 0 and 1`);
        }
      }
    }
