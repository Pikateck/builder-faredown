...
class ScoringEngine {
  ...
  async scoreCandidates(context, feasibleActions) {
    ...
    try {
      ...
      const candidates = feasibleActions.actions.map(action => ({
        ...
        features: this.prepareCandidateFeatures(action, baseFeatures, feasibleActions)
      }));

      // Step 3: Batch model inference (3-8ms)
      const inferenceStart = Date.now();
      const featureMatrix = candidates.map(c => c.features);
      const acceptanceProbabilities = await this.propensityModel.predictBatch(featureMatrix);
      ...
      this.updateTelemetry(totalTime, featurePrepTime, inferenceTime, scoringTime);
  ...
  prepareCandidateFeatures(action, baseFeatures, feasibleActions) {
    ...
    const discountDepth = priceRange > 0 ? (maxPrice - action.price) / priceRange : 0;
    
    // Relative position in price range
    const pricePosition = priceRange > 0 ? (action.price - minPrice) / priceRange : 0.5;
  ...
  calculateTrueCost(action, supplier_snapshots) {
    ...
    const baseCost = parseFloat(snapshot.net) + 
  ...
  updateTelemetry(totalTime, featurePrepTime, inferenceTime, scoringTime) {
    ...
    this.telemetry.avg_time_ms = this.telemetry.total_time_ms / this.telemetry.total_calls;
  ...
  getPerformanceMetrics() {
    return {
      ...this.telemetry,
      avg_feature_prep_ms: this.telemetry.feature_prep_time / this.telemetry.total_calls,
      avg_inference_ms: this.telemetry.inference_time / this.telemetry.total_calls,
