...
class BudgetMonitorService {
  ...
  async sendBudgetAlert(promo, level, utilizationPercent) {
    ...
    // Log alert
    console.log(`[BUDGET ALERT - ${level}] Promo ${promo.code}: ${utilizationPercent.toFixed(2)}% budget used`);
  ...
  async autoDisablePromo(promo, utilizationPercent) {
    try {
      ...
      if (updatedPromo) {
        console.log(`[AUTO-DISABLE] Promo ${promo.code} automatically disabled - budget exhausted`);
  ...
  async checkForRecovery() {
    try {
      ...
      for (const promo of promoCodes) {
        if (promo.status === 'exhausted' && promo.disabledBy === 'system_auto') {
          const utilizationPercent = (promo.budgetUsed / promo.marketingBudget) * 100;
  ...
  async autoRecoverPromo(promo, utilizationPercent) {
    try {
      ...
      if (updatedPromo) {
        console.log(`[AUTO-RECOVERY] Promo ${promo.code} automatically recovered - budget increased`);
  ...
  async generateDailyReport() {
    ...
    const report = {
      ...
      summary: {
        ...
        activePromoCodes: promoCodes.filter(p => p.status === 'active').length,
        exhaustedPromoCodes: promoCodes.filter(p => p.status === 'exhausted').length,
      ...
      promoDetails: promoCodes.map(promo => ({
        ...
        budgetUsed: promo.budgetUsed,
        budgetRemaining: promo.marketingBudget - promo.budgetUsed,
        utilizationPercent: ((promo.budgetUsed / promo.marketingBudget) * 100).toFixed(2),
        usageCount: promo.usageCount
      })),
    ...
    console.log(`Total Budget: ₹${report.summary.totalBudget.toLocaleString()}`);
    console.log(`Budget Used: ₹${report.summary.totalBudgetUsed.toLocaleString()} (${report.summary.budgetUtilization}%)`);
    console.log(`Active Promos: ${report.summary.activePromoCodes}/${report.summary.totalPromoCodes}`);
  ...
  async checkPromocodeBudget(promoId) {
    const promo = PromoCodeValidator.getAllPromoCodes().find(p => p.id === promoId);
    if (promo) {
      await this.checkBudget(promo);
      return {
        promoId,
