...
class LoyaltyService {
  ...
  async processEarning(bookingData) {
    try {
      console.log(`🎯 Processing loyalty earning for booking ${bookingData.bookingId}`);
      
      // Validate required fields
      if (!bookingData.userId || !bookingData.bookingId || !bookingData.bookingType) {
      ...
      const response = await fetch(`${this.serverUrl}/api/loyalty/process-earning`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          eligibility: {
            eligibleAmount: parseFloat(bookingData.eligibility.eligibleAmount),
            currency: bookingData.eligibility.currency || 'INR',
            fxRate: parseFloat(bookingData.eligibility.fxRate || 1.0)
          },
          description: bookingData.description || `${bookingData.bookingType} booking earning`
        })
      });

      if (!response.ok) {
        const errorText = await response.text();
        console.error(`❌ Loyalty service error: ${response.status} - ${errorText}`);
      ...
      if (result.success) {
        console.log(`✅ Loyalty earning processed: ${result.data.earnedPoints} points earned`);
        if (result.data.tierUpdate) {
          console.log(`🏆 Member tier updated: ${result.data.tierUpdate.oldTier} → ${result.data.tierUpdate.newTier}`);
        }
      } else {
  ...
  async calculateEarning(amount, bookingType, currency = 'INR', fxRate = 1.0) {
    try {
      const response = await fetch(`${this.serverUrl}/api/loyalty/calculate-earning`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error(`❌ Points calculation error: ${response.status} - ${errorText}`);
        return { success: false, error: `HTTP ${response.status}: ${errorText}` };
      }
