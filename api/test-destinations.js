  ...
  for (const query of testQueries) {
    try {
      ...
      if (destinations.length > 0) {
        console.log(`✅ Found ${destinations.length} destinations for "${query}"`);
        ...
        try {
          ...
          if (hotels && hotels.length > 0) {
            ...
            hotels.slice(0, 2).forEach(hotel => {
              console.log(`   🏨 ${hotel.name} - ${hotel.categoryName || 'N/A'}`);
            });
          ...
          console.log(`❌ Hotel search failed for ${destCode}: ${hotelError.message}`);
        }
      } else {
