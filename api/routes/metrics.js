  ...
  return (req, res, next) => {
    ...
    res.on('finish', () => {
      ...
      if (route && route.startsWith('/session')) {
        ...
        httpRequestTotal.labels(route, res.statusCode.toString(), req.method).inc();
      }
    });
...
async function updateMetricsFromCache(redisClient) {
  try {
    ...
    if (versionInfo) {
      ...
      policyVersion.labels(versions.policy_version || 'unknown', process.env.NODE_ENV || 'development').set(1);
      modelVersion.labels(versions.model_version || 'unknown', 'pricing', process.env.NODE_ENV || 'development').set(1);
    }
