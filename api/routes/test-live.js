...
router.get('/hotelbeds', async (req, res) => {
  try {
    ...
    if (results.tests[0].status === 'success' && results.tests[0].data.count > 0) {
      try {
        ...
        const hotels = await hotelbedsService.searchHotelAvailability(searchParams);
        results.tests.push({
    ...
    const allSuccessful = results.tests.every(test => test.status === 'success');
    
    res.json({
      success: allSuccessful,
      message: allSuccessful ? 'All Hotelbeds API tests passed' : 'Some tests failed',
      data: results
    });
...
router.get('/full', async (req, res) => {
  try {
    ...
    // Test Hotelbeds
    try {
      const hotelbedsResponse = await fetch(`${req.protocol}://${req.get('host')}/api/test-live/hotelbeds`);
    ...
    // Test GIATA
    try {
      const giataResponse = await fetch(`${req.protocol}://${req.get('host')}/api/test-live/giata`);
    ...
    res.json({
      success: allSuccessful,
      message: allSuccessful ? 'All live APIs operational' : 'Some APIs have issues',
      data: results
    });
