...
async function transformHotelbedsData(hotelbedsData, searchParams) {
  ...
  const transformedHotels = await Promise.all(hotels.map(async (hotel, index) => {
    // Get base price from the first available room
    const firstRoom = hotel.rooms?.[0];
    const basePrice = firstRoom ? parseFloat(firstRoom.rates?.[0]?.net || 0) : 0;
    ...
    const promoResult = await applyHotelPromoCode(markedUpPrice, searchParams.promoCode, searchParams.userId);
    ...
    const transformedHotel = {
      ...
      roomTypes: (hotel.rooms || []).map((room, roomIndex) => ({
        ...
        occupancy: {
          ...
          maxTotal: (room.paxes?.[0]?.adults || 2) + (room.paxes?.[0]?.children || 0)
        },
        bedTypes: [],
        amenities: [],
        images: [],
        rates: room.rates?.map((rate, rateIndex) => {
          ...
          const roomMarkedUpPrice = applyHotelMarkup(roomBasePrice, markupData);
...
async function saveHotelSearchToDatabase(searchParams, results) {
  try {
    ...
    console.log(`💾 Hotel search saved to database with ID: ${result.rows[0].id}`);
...
router.get("/search", async (req, res) => {
  try {
    ...
    console.log("📡 Calling Hotelbeds API with request:", JSON.stringify(hotelbedsRequest, null, 2));
    ...
    console.log(`✅ Hotelbeds API returned ${hotelbedsResponse.data?.hotels?.length || 0} hotels`);

    // Transform Hotelbeds data to our format with markup and promo codes
    const searchParams = { ...req.query };
    const transformedHotels = await transformHotelbedsData(hotelbedsResponse.data, searchParams);
  ...
  return [
    {
      ...
      amenities: [
        ...
        { id: "pool", name: "Swimming Pool", icon: "waves", category: "wellness", available: true },
        { id: "gym", name: "Fitness Center", icon: "dumbbell", category: "wellness", available: true }
      ],
      priceRange: {
        min: 15000,
        max: 25000,
