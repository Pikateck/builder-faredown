...
function validateFlagValue(flagName, value) {
  ...
  switch (metadata.type) {
    ...
    case 'float':
      ...
      if (metadata.range && (value < metadata.range[0] || value > metadata.range[1])) {
        return { valid: false, error: `${flagName} must be between ${metadata.range[0]} and ${metadata.range[1]}` };
      }
      break;
    ...
    case 'string':
      ...
      if (metadata.values && !metadata.values.includes(value)) {
        return { valid: false, error: `${flagName} must be one of: ${metadata.values.join(', ')}` };
      }
      break;
  }
...
async function setFlag(flagName, value, ttl = 0) {
  ...
  await redisClient.lpush('audit:flag_changes', JSON.stringify({
    flag: flagName,
...
router.post('/set', async (req, res) => {
  try {
    ...
    if (!flag || value === undefined) {
      return res.status(400).json({ error: 'Missing required fields: flag, value' });
    }
...
router.post('/batch-set', async (req, res) => {
  try {
    ...
    if (!flags || typeof flags !== 'object') {
      return res.status(400).json({ error: 'Missing required field: flags (object)' });
    }
...
router.get('/meta/documentation', async (req, res) => {
  try {
    ...
    res.json({
      ...
      rollout_guide: {
        phases: [
          { phase: 'shadow', ai_traffic: 0.0, ai_shadow: true, description: 'Log predictions only' },
          { phase: 'canary', ai_traffic: 0.1, ai_shadow: true, description: '10% traffic with monitoring' },
          { phase: 'partial', ai_traffic: 0.5, ai_shadow: false, description: '50% traffic rollout' },
          { phase: 'full', ai_traffic: 1.0, ai_shadow: false, description: 'Full production traffic' }
        ]
      }
    });
  } catch (error) {
...
router.get('/audit/history', async (req, res) => {
  try {
    ...
    const history = await redisClient.lrange('audit:flag_changes', 0, limit - 1);
...
function aiTrafficGate(req, res, next) {
  getFlag('AI_TRAFFIC').then(aiTraffic => {
