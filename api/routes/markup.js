  ...
  try {
    ...
    if (search) {
      filteredMarkups = filteredMarkups.filter(markup =>
        ...
        `${markup.route.from}-${markup.route.to}`.toLowerCase().includes(search.toLowerCase())
      );
    }

    if (airline && airline !== 'all') {
      filteredMarkups = filteredMarkups.filter(markup => markup.airline === airline);
    }

    if (cabinClass && cabinClass !== 'all') {
      filteredMarkups = filteredMarkups.filter(markup => markup.class === cabinClass);
    }

    if (status && status !== 'all') {
      filteredMarkups = filteredMarkups.filter(markup => markup.status === status);
    }
...
router.get('/hotel', authenticateToken, (req, res) => {
  try {
    ...
    if (category && category !== 'all') {
      filteredMarkups = filteredMarkups.filter(markup => markup.roomCategory === category);
    }
...
router.post('/:type/:id/toggle-status', authenticateToken, (req, res) => {
  try {
    ...
    markups[markupIndex].status = markups[markupIndex].status === 'active' ? 'inactive' : 'active';
