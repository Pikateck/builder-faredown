...
router.get('/', authenticateToken, (req, res) => {
  try {
    const { search, serviceType, country, status, page = 1, limit = 10 } = req.query;
    ...
    if (serviceType && serviceType !== 'all') {
      filteredRules = filteredRules.filter(rule => rule.serviceType === serviceType);
    }
...
router.put('/:id', authenticateToken, (req, res) => {
  try {
    ...
    // If setting as default, remove default flag from other rules of same service type and country
    if (isDefault && (serviceType || vatRules[ruleIndex].serviceType) && (country || vatRules[ruleIndex].country)) {
      ...
      vatRules = vatRules.map((rule, index) => {
        if (index !== ruleIndex && rule.serviceType === targetServiceType && rule.country === targetCountry) {
          return { ...rule, isDefault: false };
        }
        return rule;
      });
...
router.post('/:id/toggle-status', authenticateToken, (req, res) => {
  try {
    ...
    vatRules[ruleIndex].status = vatRules[ruleIndex].status === 'active' ? 'inactive' : 'active';
...
router.post('/calculate', (req, res) => {
  try {
    ...
    let applicableRule = vatRules.find(rule =>
    ...
    if (!applicableRule) {
      applicableRule = vatRules.find(rule =>
        rule.serviceType === serviceType &&
        rule.country === country &&
...
router.get('/stats/overview', authenticateToken, (req, res) => {
  try {
    ...
    const activeRules = vatRules.filter(rule => rule.status === 'active').length;
...
router.get('/default/:serviceType/:country', (req, res) => {
  try {
    ...
    const defaultRule = vatRules.find(rule =>
      rule.serviceType === serviceType &&
      rule.country === country &&
      rule.isDefault &&
      rule.status === 'active'
