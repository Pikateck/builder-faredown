...
router.put("/", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    const {
      firstName, lastName, fullName, email, phone, phoneE164, dob,
      nationalityIso2, gender, displayName, profilePictureUrl,
      address, preferences
    } = req.body;
    ...
    const userUpdateResult = await client.query(`
      ...
      RETURNING id, uuid
    `, [
      firstName, lastName, fullName, email, phone, phoneE164,
      dob, nationalityIso2, gender, displayName, profilePictureUrl, req.userId
    ]);
    ...
    if (address) {
      const addressResult = await client.query(`
        ...
        RETURNING id
      `, [address.line1, address.line2, address.city, address.state, address.postalCode, address.countryIso2]);
    ...
    if (preferences) {
      await client.query(`
        ...
        ON CONFLICT (user_id) DO UPDATE SET
          ...
          updated_at = now()
      `, [
        req.userId, preferences.currencyIso3, preferences.language, preferences.timezone,
        preferences.dateFormat, preferences.marketingOptIn, preferences.emailNotifications,
        preferences.smsNotifications, preferences.pushNotifications, preferences.priceAlerts,
        preferences.bookingReminders, preferences.travelTips
      ]);
    }
...
router.post("/travelers", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    const {
      firstName, lastName, dob, gender, nationalityIso2, relationship,
      frequentFlyerNumber, dietaryRestrictions, mobilityAssistance,
      isPrimary, address
    } = req.body;
    ...
    `, [
      req.userId, firstName, lastName, dob, gender, nationalityIso2,
      relationship, frequentFlyerNumber, dietaryRestrictions,
      mobilityAssistance, isPrimary, addressId
    ]);
...
router.get("/travelers/:travelerId/passports", authenticateUser, setPassportKey, async (req, res) => {
  try {
...
router.post("/travelers/:travelerId/passports", authenticateUser, setPassportKey, async (req, res) => {
  try {
    await req.dbClient.query('BEGIN');
    
    const { travelerId } = req.params;
    const {
      givenNames, surname, passportNumber, issuingCountry,
      issueDate, expiryDate, placeOfBirth, isPrimary
    } = req.body;
    
    // Validate required fields
    if (!givenNames || !surname || !passportNumber || !issuingCountry || !expiryDate) {
    ...
    `, [
      travelerId, givenNames, surname, passportNumber, issuingCountry,
      issueDate, expiryDate, placeOfBirth, isPrimary
    ]);
...
router.post("/payment-methods", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    const {
      provider, token, type, brand, last4, expMonth, expYear,
      holderName, isDefault, billingAddress
    } = req.body;
    
    // Validate required fields
    if (!provider || !token || !type) {
      return res.status(400).json({ error: "Provider, token, and type are required" });
    }
    ...
    if (billingAddress) {
      ...
      `, [
        ...
        billingAddress.state, billingAddress.postalCode, billingAddress.countryIso2
      ]);
    ...
    `, [
      req.userId, provider, token, type, brand, last4, expMonth, expYear,
      holderName, billingAddressId, isDefault
    ]);
...
router.put("/payment-methods/:id/default", authenticateUser, async (req, res) => {
  let client;
  try {
    client = await pool.connect();
    await client.query('BEGIN');
...
router.put("/preferences", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    const result = await client.query(`
      ...
      RETURNING *
    `, [
      preferences.currencyIso3, preferences.language, preferences.timezone,
      preferences.dateFormat, preferences.marketingOptIn, preferences.emailNotifications,
      preferences.smsNotifications, preferences.pushNotifications, preferences.priceAlerts,
      preferences.bookingReminders, preferences.travelTips, req.userId
    ]);
...
router.get("/booking-passengers/:bookingId", authenticateUser, async (req, res) => {
  let client;
  try {
    client = await pool.connect();
...
router.post("/booking-passengers", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    const result = await client.query(`
      ...
      RETURNING *
    `, [
      bookingId, travelerId, traveler.first_name, traveler.last_name,
      traveler.dob, traveler.gender, traveler.nationality_iso2,
      traveler.passport_last4, traveler.frequent_flyer_number
    ]);
