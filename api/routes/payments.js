...
router.post('/verify', async (req, res) => {
  try {
    ...
    if (!razorpay_order_id || !razorpay_payment_id || !razorpay_signature || !tempBookingRef) {
      return res.status(400).json({
        success: false,
    ...
    // Confirm booking
    const bookingResult = await hotelBookingService.confirmBooking(tempBookingRef, {
      razorpay_payment_id,
      razorpay_order_id,
      razorpay_signature,
      method: paymentDetails.payment?.method || 'card',
      amount: paymentDetails.payment?.amount ? paymentDetails.payment.amount / 100 : 0,
