...
router.post("/:id/markup", authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    const { module, markup_type, markup_value, valid_from, valid_to, status = true, acted_by } = req.body || {};

    if (!module || !markup_type || markup_value == null) {
      return res.status(400).json({ success: false, error: "module, markup_type, markup_value required" });
    }
    ...
    const ins = await db.query(
      ...
      [id, String(module).toUpperCase(), markup_type.toUpperCase(), Number(markup_value), valid_from || null, valid_to || null, !!status, acted_by || 'admin']
    );
