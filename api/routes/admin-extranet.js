...
router.get("/inventory", requireAdmin, async (req, res) => {
  try {
    ...
    if (status && status !== "all") {
      filteredInventory = filteredInventory.filter(
        (item) => item.availability.status === status,
      );
    }
  ...
  try {
    ...
    if (module && module !== "all") {
      ...
      const totalRatingSum = Object.values(inventoryStats).reduce(
        (sum, stats) => sum + stats.avg_rating * stats.total_items,
        0,
      );
      aggregatedStats.avg_rating =
        aggregatedStats.total_items > 0
    ...
    try {
      ...
      if (!module || !title || !description || !location || !pricing) {
        return res.status(400).json({
          success: false,
          message:
            "Missing required fields: module, title, description, location, pricing",
        });
      }
 ...
 */
router.patch(
  "/inventory/:id/toggle-status",
  requireAdmin,
  auditLog("extranet_toggle"),
  async (req, res) => {
    try {
      const { id } = req.params;
          ...
          const newItem = {
            ...
            details: {
              includes: item.includes
                ? item.includes.split(",").map((s) => s.trim())
                : [],
              excludes: item.excludes
                ? item.excludes.split(",").map((s) => s.trim())
                : [],
              highlights: item.highlights
                ? item.highlights.split(",").map((s) => s.trim())
                : [],
            },
            metadata: {
              created_at: new Date().toISOString(),
