...
router.post('/check', authenticateToken, validate.checkBargain, trackPromoUsage, async (req, res) => {
  try {
    ...
    const markupAmount = markupRange.type === 'percent' 
    ...
    minDiscountedPrice = Math.max(minDiscountedPrice, baseNetPrice + (baseNetPrice * 0.02)); // 2% minimum margin
    
    const sessionId = `bargain_${Date.now()}_${req.user.id}`;
    
    // Check if user price is within acceptable range
    if (userInputPrice >= minDiscountedPrice && userInputPrice <= maxDiscountedPrice) {
      // Accept the bargain
      const session = {
        id: sessionId,
        userId: req.user.id,
      ...
      res.json({
        ...
        priceBreakdown: {
          ...
          savingsPercent: ((priceWithMarkup - userInputPrice) / priceWithMarkup * 100).toFixed(1)
        }
      });
      
    } else if (userInputPrice < minDiscountedPrice) {
      // User price is too low, offer counter
      const counterOffer = Math.round(minDiscountedPrice + ((maxDiscountedPrice - minDiscountedPrice) * 0.3));
...
router.post('/counter', authenticateToken, validate.counterOffer, async (req, res) => {
  try {
    ...
    await audit.userAction(req, 'bargain_counter', { sessionId, newPrice, action });
    ...
    if (action === 'accept') {
      ...
      if (session.attempts > 3) {
        ...
        res.json({
          success: false,
          status: 'expired',
          message: 'Maximum negotiation attempts reached. Book at original price.',
          originalPrice: session.originalPrice
        });
        return;
      }
      ...
      if (newPrice >= minPrice) {
        ...
        const newCounter = Math.round(minPrice + ((session.originalPrice - minPrice) * 0.2));
 ...
 */
router.get('/logs', requirePermission(PERMISSIONS.PROMO_VIEW), async (req, res) => {
  try {
    const { promoCode, status, startDate, endDate, page = 1, limit = 50 } = req.query;
    ...
    const stats = {
      ...
      exhaustedCodes: promoCodes.filter(p => p.status === 'exhausted').length,
...
router.get('/sessions', authenticateToken, async (req, res) => {
  try {
    ...
    // Get user's sessions
    const userSessions = Array.from(bargainSessions.values())
      .filter(session => session.userId === req.user.id);
    
    // Apply status filter if provided
    let filteredSessions = userSessions;
    if (status) {
      filteredSessions = userSessions.filter(session => session.status === status);
    }
    ...
    res.json({
      ...
      data: {
        ...
        savings: promoCode ? {
          ...
          minPercent: ((maxPrice - discountedMaxPrice) / maxPrice * 100).toFixed(1),
          maxPercent: ((maxPrice - discountedMinPrice) / maxPrice * 100).toFixed(1)
        } : null
      }
    });
