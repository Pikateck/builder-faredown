...
router.get("/search", async (req, res) => {
  ...
  try {
    ...
    const typesFilter = types
      .split(",")
      .map((t) => t.trim())
      .filter((t) => ["city", "country", "region"].includes(t));
    ...
    if (totalTime > 300) {
      ...
      console.warn(
        `   ðŸ“Š Breakdown: cache=${timings.cache}ms | parse=${timings.parse}ms | query=${timings.query}ms | rank=${timings.rank}ms | respond=${timings.respond}ms`,
      );
      console.warn(
        `   ðŸ“‹ Results: ${formattedResults.length} items, types: ${typesFilter.join(",")}`,
      );
    } else {
      console.log(
        `âš¡ Fast search v2: "${queryText}" = ${totalTime}ms (${formattedResults.length} results)`,
      );
    }
    ...
    console.error(
      `   ðŸ“Š Breakdown: cache=${timings.cache}ms | parse=${timings.parse}ms | query=${timings.query}ms`,
    );
...
router.get("/health", async (req, res) => {
  try {
    ...
    res.json({
      ...
      search_function:
        testResult.rows[0].count > 0 ? "working" : "empty_results",
      cache: cacheStats,
      database: "connected",
    });
  } catch (error) {
