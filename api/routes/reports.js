  ...
  try {
    ...
    if (search) {
      filteredReports = filteredReports.filter(report =>
        report.bookingReference.toLowerCase().includes(search.toLowerCase()) ||
    ...
    if (serviceType && serviceType !== 'all') {
      filteredReports = filteredReports.filter(report => report.serviceType === serviceType);
    }
    ...
    if (dateFrom) {
      filteredReports = filteredReports.filter(report => 
        new Date(report.bookingDate) >= new Date(dateFrom)
      );
    }

    if (dateTo) {
      filteredReports = filteredReports.filter(report => 
        new Date(report.bookingDate) <= new Date(dateTo)
      );
    }

    // Sort by booking date (most recent first)
    filteredReports.sort((a, b) => new Date(b.bookingDate) - new Date(a.bookingDate));
...
router.get('/analytics', authenticateToken, (req, res) => {
  try {
    ...
    const totalRevenue = bookingReports.reduce((sum, report) => sum + report.amount, 0);
    const totalCommission = bookingReports.reduce((sum, report) => sum + report.commission, 0);
    const flightBookings = bookingReports.filter(r => r.serviceType === 'flight').length;
    const hotelBookings = bookingReports.filter(r => r.serviceType === 'hotel').length;
...
router.get('/insights', authenticateToken, (req, res) => {
  try {
    ...
    const paymentMethodPercentages = Object.entries(paymentMethods).map(([method, count]) => ({
      method,
      count,
    ...
    const uniqueCustomers = new Set(bookingReports.map(r => r.customerEmail)).size;
    ...
    res.json({
      customerInsights: {
        ...
        averageBookingValue: bookingReports.reduce((sum, r) => sum + r.amount, 0) / bookingReports.length,
        customerRetention
      },
...
router.get('/dashboard', authenticateToken, (req, res) => {
  try {
    ...
    const recentBookings = bookingReports.filter(report => 
      new Date(report.bookingDate) >= thirtyDaysAgo
    );

    const totalBookings = recentBookings.length;
    const totalRevenue = recentBookings.reduce((sum, report) => sum + report.amount, 0);
    const totalCommission = recentBookings.reduce((sum, report) => sum + report.commission, 0);
    const successRate = (recentBookings.filter(r => r.status === 'confirmed').length / totalBookings) * 100;
  ...
  try {
    ...
    if (metrics && metrics.length > 0) {
      metrics.forEach(metric => {
        switch (metric) {
          case 'total_amount':
            calculatedMetrics.totalAmount = data.reduce((sum, item) => sum + (item.amount || 0), 0);
            break;
          case 'average_amount':
            calculatedMetrics.averageAmount = data.length > 0 ? 
              data.reduce((sum, item) => sum + (item.amount || 0), 0) / data.length : 0;
            break;
          case 'count':
            calculatedMetrics.count = data.length;
            break;
          case 'commission':
            calculatedMetrics.totalCommission = data.reduce((sum, item) => sum + (item.commission || 0), 0);
            break;
        }
      });
    }
