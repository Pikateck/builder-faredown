...
let markupRules = seedData.markupRules.filter(
  (rule) => rule.module === "packages",
) || [
  {
    id: "rule_001",
      ...
      filteredRules = filteredRules.filter(
        (rule) =>
          ...
          (rule.conditions?.packageCategory &&
            rule.conditions.packageCategory.includes(category)),
      );
    }
...
router.get("/stats", requireAdmin, async (req, res) => {
  try {
    ...
    const totalRevenue = markupRules.reduce(
      (sum, rule) => sum + rule.usage.revenue,
      0,
    );
    const totalApplications = markupRules.reduce(
      (sum, rule) => sum + rule.usage.totalApplications,
      0,
    );
    const avgMarkupPercentage =
      totalApplications > 0
        ? markupRules.reduce(
            (sum, rule) =>
              sum + rule.usage.avgMarkup * rule.usage.totalApplications,
            0,
          ) / totalApplications
        : 0;

    // Find top performing rule
    const topRule = markupRules.reduce(
      (top, rule) =>
        rule.usage.revenue > (top?.usage.revenue || 0) ? rule : top,
      null,
    );
...
router.post("/", requireAdmin, auditLog("markup_create"), async (req, res) => {
  try {
    ...
    const existingPriority = markupRules.find(
      (rule) => rule.priority === priority && rule.isActive,
    );
    if (existingPriority) {
      return res.status(400).json({
        success: false,
  ...
  try {
    ...
    const applicableRules = markupRules
      ...
      .filter((rule) => {
        ...
        if (
          conditions.packageCategory &&
          conditions.packageCategory.length > 0
        ) {
          if (
            !packageCategory ||
            !conditions.packageCategory.includes(packageCategory)
          ) {
            return false;
          }
        }
        ...
        if (conditions.priceRange) {
          if (
            ...
            basePrice < conditions.priceRange.min
          ) {
            return false;
          }
          if (
            conditions.priceRange.max &&
            basePrice > conditions.priceRange.max
          ) {
            return false;
          }
        }
        ...
        if (
          conditions.advanceBooking &&
          (!advanceBooking || advanceBooking < conditions.advanceBooking)
        ) {
          return false;
        }
        ...
        if (conditions.groupSize) {
          if (
            ...
            (!groupSize || groupSize < conditions.groupSize.min)
          ) {
            return false;
          }
          if (
            conditions.groupSize.max &&
            (!groupSize || groupSize > conditions.groupSize.max)
          ) {
            return false;
          }
        }
    ...
    if (applicableRules.length > 0) {
      ...
      appliedRules.push({
        ...
        markupPercentage:
          Math.round((markupAmount / basePrice) * 100 * 10) / 10,
      });
    }
