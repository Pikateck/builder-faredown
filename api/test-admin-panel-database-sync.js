...
async function checkExistingData() {
  try {
    ...
    if (airSample.rows.length > 0) {
      ...
      console.log('      Status:', airSample.rows[0].status ? 'Active' : 'Inactive');
...
// Run tests
runAllTests().then(({ testsPassed, testsFailed }) => {
  console.log('\n' + '='.repeat(80));
  ...
  if (testsFailed === 0) {
    ...
    console.log('✅ Admin panel changes will reflect in PgAdmin immediately\n');
    process.exit(0);
  } else {
