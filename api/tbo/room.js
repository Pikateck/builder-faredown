...
async function getHotelRoom(params = {}) {
  ...
  if (!traceId || !resultIndex || !hotelCode) {
    throw new Error("Missing required parameters: traceId, resultIndex, hotelCode");
  }
  ...
  const url = process.env.TBO_HOTEL_ROOM_URL || "https://hotelbooking.travelboutiqueonline.com/HotelAPI_V10/HotelService.svc/rest/GetHotelRoom";
  ...
  if (response.data?.HotelRoomDetails?.length > 0) {
    ...
    response.data.HotelRoomDetails.slice(0, 3).forEach((r, i) => {
      console.log(`  ${i + 1}. ${r.RoomTypeName} - ${r.Price?.CurrencyCode} ${r.Price?.OfferedPrice}`);
