...
function priceEcho(options = {}) {
  ...
  return async function priceEchoMiddleware(req, res, next) {
    ...
    const step = String(req.headers[stepHeader] || '').toLowerCase().trim();
    ...
    async function capturePriceData(responseBody, currentStep, currentJourneyId) {
      try {
        ...
        await logPriceCheckpoint(currentJourneyId, currentStep, priceData, responseBody);
    ...
    async function logPriceCheckpoint(journeyId, step, priceData, payload) {
      ...
      console.log(`[PriceEcho] Logged ${step} price: ${priceData.currency} ${priceData.totalFare} for journey ${journeyId}`);
    }

    async function checkPriceMismatch(journeyId, currentStep, currentPriceData) {
      try {
        ...
        if (hasMismatch) {
          ...
          console.log(`[PriceEcho] Expected price change in bargain_post: ${delta > 0 ? '+' : ''}${delta}`);
        }
  ...
  return async function diffEndpoint(req, res) {
    try {
      ...
      const steps = result.rows.map(row => ({
        ...
        deltaFromFirst: Number((Number(row.total_fare) - firstPrice).toFixed(2)),
      ...
      res.json({
        ...
        priceChanges: steps.filter(s => Math.abs(s.deltaFromFirst) > 0.01).length
      });
