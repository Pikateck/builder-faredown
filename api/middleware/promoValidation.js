...
class PromoCodeValidator {
  ...
  static validatePromoCode(promoCode, bookingDetails, userId = null) {
    ...
    // Check applicability
    if (promo.applicableTo !== 'both' && promo.applicableTo !== bookingDetails.type) {
  ...
  static validateFilters(promo, bookingDetails) {
    ...
    if (bookingDetails.type === 'flight') {
      // Flight filters
      if (promo.filters.fromCity && bookingDetails.fromCity !== promo.filters.fromCity) {
        result.isValid = false;
        result.message = `Must depart from ${promo.filters.fromCity}`;
        result.errors.push('FROM_CITY_MISMATCH');
      }

      if (promo.filters.toCity && bookingDetails.toCity !== promo.filters.toCity) {
        result.isValid = false;
        result.message = `Must arrive at ${promo.filters.toCity}`;
        result.errors.push('TO_CITY_MISMATCH');
      }

      if (promo.filters.airlines && bookingDetails.airline && 
      ...
      if (promo.filters.cabinClass && bookingDetails.cabin && 
      ...
      // Hotel filters
      if (promo.filters.cities && bookingDetails.city && 
      ...
      if (promo.filters.hotels && bookingDetails.hotel && 
      ...
      if (promo.filters.roomCategories && bookingDetails.roomCategory && 
  ...
  static getUserUsageCount(promoCode, userId) {
    return promoUsageLog.filter(log => 
   ...
   */
  static async recordUsage(promoCode, userId, bookingDetails, discountAmount, sessionId) {
    const promo = this.findPromoCode(promoCode);
  ...
  static getPromoStatistics(promoCode = null) {
    if (promoCode) {
      ...
      const usageLogs = promoUsageLog.filter(log => log.promoCode === promo.code);
      ...
      return {
        ...
        budgetUtilization: ((promo.budgetUsed / promo.marketingBudget) * 100).toFixed(2),
        totalDiscount,
        uniqueUsers,
        averageDiscount: usageLogs.length ? (totalDiscount / usageLogs.length).toFixed(2) : 0,
    ...
    return {
      ...
      averageDiscountPerUse: totalUsage ? (totalUsed / totalUsage).toFixed(2) : 0
    };
  }
  ...
  static getUsageLogs(filters = {}) {
    ...
    if (filters.promoCode) {
      logs = logs.filter(log => log.promoCode.toLowerCase().includes(filters.promoCode.toLowerCase()));
    }
    ...
    if (filters.bookingType) {
      logs = logs.filter(log => log.bookingDetails.type === filters.bookingType);
    }
...
const validatePromoCode = (req, res, next) => {
  try {
    ...
    const validation = PromoCodeValidator.validatePromoCode(promoCode, bookingDetails, userId);
...
const checkBudgetLimits = (req, res, next) => {
  try {
    ...
    const exhaustedPromos = PromoCodeValidator.getAllPromoCodes()
