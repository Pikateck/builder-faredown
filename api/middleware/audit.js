...
const createAuditEntry = (actionType, req, additionalData = {}) => {
  ...
  const user = req.user || { id: 'anonymous', username: 'anonymous', role: 'guest' };
...
const audit = {
  ...
  bookingAction: (req, action, bookingId, changes = {}) => {
    ...
    return logAuditEvent(actionTypes[action] || ACTION_TYPES.BOOKING_VIEW, req, {
      bookingId,
      changes
    });
  },
  ...
  paymentAction: (req, action, paymentData) => {
    ...
    return logAuditEvent(actionTypes[action] || ACTION_TYPES.PAYMENT_VIEW, req, {
  ...
  contentAction: (req, action, contentData) => {
    ...
    return logAuditEvent(actionTypes[action] || ACTION_TYPES.CONTENT_CREATE, req, {
  ...
  systemAction: (req, action, details = {}) => {
    ...
    return logAuditEvent(actionTypes[action] || ACTION_TYPES.CONFIG_UPDATE, req, details);
  },
...
const getAuditStats = (timeframe = '24h') => {
  ...
  recentEntries.forEach(entry => {
    ...
    // Count by user
    stats.byUser[entry.user.username] = (stats.byUser[entry.user.username] || 0) + 1;
    
    // Count by IP
    stats.byIP[entry.client.ip] = (stats.byIP[entry.client.ip] || 0) + 1;
    
    // Count errors
    if (entry.actionType === ACTION_TYPES.ERROR || (entry.response && entry.response.statusCode >= 400)) {
      stats.errors++;
    }
  });
