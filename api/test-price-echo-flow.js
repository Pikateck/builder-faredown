...
async function testE2EFlow() {
  ...
  try {
    ...
    const steps = [
      ...
      { step: "bargain_post", baseFare: 450.0, promoCode: "WELCOME10" }, // Price changed after bargain
      { step: "book", baseFare: 450.0, promoCode: "WELCOME10" },
      { step: "payment", baseFare: 450.0, promoCode: "WELCOME10" },
      { step: "invoice", baseFare: 450.0, promoCode: "WELCOME10" },
      { step: "my_trips", baseFare: 450.0, promoCode: "WELCOME10" },
    ];
    ...
    for (const { step, baseFare, promoCode } of steps) {
      ...
      console.log(
        `${step.padEnd(15)} → ${quote.currency} ${quote.totalFare} (base: ${quote.baseFare}, markup: ${quote.markup}, discount: ${quote.discount}, tax: ${quote.tax})`,
      );
    }
    ...
    priceCheck.rows.forEach((row) => {
      ...
      const status =
        delta === 0 ? "✅" : row.step === "bargain_post" ? "🔄" : "⚠️";
      console.log(
        `   ${status} ${row.step.padEnd(15)}: ${row.total_fare} (Δ${delta > 0 ? "+" : ""}${delta || 0})`,
      );
    });

    console.log("\n🎉 End-to-End flow test completed successfully!");
    console.log(
      `📈 Price tracking logged ${priceCheck.rows.length} checkpoints`,
    );
