...
console.log(`   RATEHAWK_API_ID: ${process.env.RATEHAWK_API_ID ? '✅ Set' : '❌ Missing'}`);
console.log(`   RATEHAWK_API_KEY: ${process.env.RATEHAWK_API_KEY ? '✅ Set' : '❌ Missing'}`);
console.log(`   RATEHAWK_BASE_URL: ${process.env.RATEHAWK_BASE_URL || '❌ Using default'}`);
console.log(`   HOTELS_SUPPLIERS: ${process.env.HOTELS_SUPPLIERS || '❌ Missing'}`);
console.log(`   HOTELBEDS_API_KEY: ${process.env.HOTELBEDS_API_KEY ? '✅ Set' : '❌ Missing'}`);
...
adapters.forEach((adapter, i) => {
  console.log(`   ${i + 1}. ${adapter.supplierCode} (${adapter.constructor.name})`);
});
...
if (rateHawkAdapter) {
  console.log(`   ✅ RATEHAWK adapter found: ${rateHawkAdapter.constructor.name}`);
...
if (hotelbedsAdapter) {
  console.log(`   ✅ HOTELBEDS adapter found: ${hotelbedsAdapter.constructor.name}`);
...
(async () => {
  try {
    ...
    console.log(`   Searching for: ${testParams.destination}, ${testParams.checkIn} to ${testParams.checkOut}`);
    const result = await supplierAdapterManager.searchAllHotels(testParams, ['HOTELBEDS', 'RATEHAWK']);
    ...
    Object.entries(result.supplierMetrics).forEach(([supplier, metrics]) => {
      ...
      console.log(`      ${status} ${supplier}: ${metrics.resultCount} results (${metrics.responseTime}ms)`);
      if (metrics.error) {
