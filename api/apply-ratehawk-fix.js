  ...
  try {
    ...
    try {
      ...
      if (e.message.includes('does not exist')) {
        console.log('   ⚠️  ai.suppliers table not found, creating structure...');
      } else {
        console.log('   ⚠️  Column already exists or other issue:', e.message.split('\n')[0]);
      }
    }
    ...
    const rateHawkResult = await pool.query(`
      ...
      RETURNING *
    `);
    console.log(`   ✅ RateHawk record: ID=${rateHawkResult.rows[0].id}, Code=${rateHawkResult.rows[0].code}`);
    ...
    const hotelbedsResult = await pool.query(`
      ...
      RETURNING *
    `);
    console.log(`   ✅ Hotelbeds record: ID=${hotelbedsResult.rows[0].id}, Code=${hotelbedsResult.rows[0].code}`);
    ...
    aiSuppliersResult.rows.forEach(s => {
      console.log(`      ${s.id}. ${s.code}: ${s.name} (${s.product_type}) - Active: ${s.active ? '✅' : '❌'}`);
    });
    ...
    console.log('Summary:');
    console.log('  - RateHawk added to ai.suppliers (will now store search results)');
    console.log('  - All suppliers verified in both ai and public schemas');
    console.log('  - Next search should return RateHawk results alongside Hotelbeds');
    console.log('\n');
