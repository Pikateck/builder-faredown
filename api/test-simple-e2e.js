...
async function testSimpleE2E() {
  ...
  try {
    ...
    console.log(
      `   💰 Price: ${searchQuote.currency} ${searchQuote.totalFare}`,
    );
    ...
    const bargainQuote = await pricingEngine.quote({
      ...
      airlineCode: "AI",
      currency: "USD",
      baseFare: 450.0, // Reduced after bargain
      userType: "b2c",
      extras: { promoCode: "BUSINESSDEAL" },
    });
    console.log(
      `   💰 Price: ${bargainQuote.currency} ${bargainQuote.totalFare} (discount: ${bargainQuote.discount})`,
    );
    ...
    console.log(
      `   💰 Price: ${paymentQuote.currency} ${paymentQuote.totalFare}`,
    );
    ...
    journey.rows.forEach((row, i) => {
      ...
      const delta = row.prev_fare
        ? (row.total_fare - row.prev_fare).toFixed(2)
        : 0;
      const deltaFromBaseline = (row.total_fare - baselinePrice).toFixed(2);

      console.log(
        `   ${row.step.padEnd(15)}: ${row.total_fare} (Δ${delta > 0 ? "+" : ""}${delta}, Total Δ${deltaFromBaseline > 0 ? "+" : ""}${deltaFromBaseline})`,
      );
    });
