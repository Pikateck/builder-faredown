...
class DatabaseInitializer {
  ...
  async initialize() {
    ...
    try {
      ...
      if (schemaExists) {
        ...
        const choice = await this.promptUser('Do you want to recreate the schema? This will delete all existing data. (y/N): ');
  ...
  async dropSchema() {
    ...
    // Drop functions
    try {
      await this.pool.query('DROP FUNCTION IF EXISTS search_destinations CASCADE');
    ...
    // Drop views
    try {
      await this.pool.query('DROP VIEW IF EXISTS destinations_search_view CASCADE');
  ...
  async createSchema() {
    ...
    try {
      // Read schema file
      const schemaPath = path.join(__dirname, '../database/schema/destinations.sql');
  ...
  async verifySetup() {
    ...
    // Check tables
    const tables = ['countries', 'destinations', 'hotels_cache', 'hotel_rooms_cache', 'destination_searches'];
    for (const table of tables) {
      const result = await this.pool.query(`
  ...
  async showStatistics() {
    try {
      // Countries count
      const countries = await this.pool.query('SELECT COUNT(*) as count FROM countries');
      console.log(`   Countries: ${countries.rows[0].count}`);
      
      // Destinations count
      const destinations = await this.pool.query('SELECT COUNT(*) as count FROM destinations');
      console.log(`   Destinations: ${destinations.rows[0].count}`);
      
      // Popular destinations
      const popular = await this.pool.query('SELECT COUNT(*) as count FROM destinations WHERE popular = true');
      ...
      samples.rows.forEach(dest => {
        ...
        console.log(`   ${star} ${dest.hotelbeds_code} - ${dest.name}, ${dest.country_name}`);
      });
