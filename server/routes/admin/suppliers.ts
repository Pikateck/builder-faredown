...
import { sendSuccess, sendError, pool, handleDatabaseError } from '../../utils/adminUtils';
...
router.get('/analytics', requirePermission(Permission.VIEW_SUPPLIERS), async (req: AuthenticatedRequest, res) => {
  try {
...
router.get('/sync-logs', requirePermission(Permission.VIEW_SUPPLIERS), async (req: AuthenticatedRequest, res) => {
  try {
    const { supplierId, limit = 50 } = req.query;

    console.log(`📋 Admin: Getting sync logs... supplier: ${supplierId}, limit: ${limit}`);
