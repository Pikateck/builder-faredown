...
import { AuthenticatedRequest, requirePermission, Permission, AdminRole } from '../../middleware/adminAuth';
import { 
  pool, 
  sendSuccess, 
  sendError, 
...
router.get('/', requirePermission(Permission.VIEW_USERS), async (req: AuthenticatedRequest, res) => {
  try {
    const params = parsePaginationParams(req.query);
    const { status, role } = req.query;
    
    console.log(`👥 Getting users list - page ${params.page}, pageSize ${params.pageSize}`);
    ...
    const orderBy = params.sort === 'name' ? 'u.first_name' : `u.${params.sort}`;
...
router.get('/:id', requirePermission(Permission.VIEW_USERS), async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
...
router.post('/', requirePermission(Permission.MANAGE_USERS), async (req: AuthenticatedRequest, res) => {
  try {
...
router.put('/:id', requirePermission(Permission.MANAGE_USERS), async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
    const userData = sanitizeInput(req.body);
    ...
    // Validate input
    const validationErrors = validateUserData({ ...existingUser, ...userData });
    if (validationErrors.length > 0) {
      return sendValidationError(res, validationErrors);
    }
    ...
    if (userData.email && userData.email !== existingUser.email) {
      const emailCheck = await pool.query('SELECT id FROM users WHERE email = $1 AND id != $2', [userData.email, id]);
      if (emailCheck.rows.length > 0) {
        return sendError(res, 409, 'EMAIL_EXISTS', 'Email already in use by another user');
      }
    }
...
router.patch('/:id/status', requirePermission(Permission.MANAGE_USERS), async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
...
router.delete('/:id', requirePermission(Permission.MANAGE_USERS), async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
...
router.get('/stats/summary', requirePermission(Permission.VIEW_USERS), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const [
      ...
      pool.query("SELECT COUNT(*) as new_users FROM users WHERE created_at >= NOW() - INTERVAL '30 days'"),
    ...
    const stats = {
      ...
      inactiveUsers: parseInt(totalUsersResult.rows[0]?.total || '0') - parseInt(activeUsersResult.rows[0]?.active || '0'),
