...
router.get('/', requirePermission(Permission.VIEW_BOOKINGS), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    console.log(`📚 Getting bookings list - page ${params.page}, pageSize ${params.pageSize}`);
    ...
    const orderBy = params.sort === 'guest' ? 'u.first_name' : 
                   params.sort === 'hotel' ? 'hb.hotel_name' :
                   `hb.${params.sort}`;
...
router.get('/:id', requirePermission(Permission.VIEW_BOOKINGS), async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
...
router.patch('/:id/status', requirePermission(Permission.MANAGE_BOOKINGS), async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
    ...
    const existingResult = await pool.query('SELECT * FROM hotel_bookings WHERE id = $1', [id]);
...
router.post('/:id/resend-voucher', requirePermission(Permission.MANAGE_BOOKINGS), async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
...
router.get('/stats/summary', requirePermission(Permission.VIEW_BOOKINGS), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const stats = {
      ...
      revenue: {
        ...
        average: parseFloat(revenueStatsResult.rows[0]?.avg_booking_value || '0'),
