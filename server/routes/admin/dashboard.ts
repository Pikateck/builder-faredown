...
import { pool, sendSuccess, sendError, validateDateRange, formatCurrency } from '../../utils/adminUtils';

const router = express.Router();

// Dashboard summary endpoint
router.get('/summary', requirePermission(Permission.VIEW_DASHBOARD), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    console.log(`📊 Getting dashboard summary from ${startDate.toISOString()} to ${endDate.toISOString()}`);
    ...
    // Process results
    const totalBookings = parseInt(totalBookingsResult.rows[0]?.total_bookings || '0');
    ...
    const previousBookings = parseInt(previousBookingsResult.rows[0]?.count || '0');
    const previousRevenue = parseFloat(previousRevenueResult.rows[0]?.amount || '0');

    const bookingGrowth = previousBookings > 0 ? 
      ((totalBookings - previousBookings) / previousBookings * 100).toFixed(1) : '0.0';
    const revenueGrowth = previousRevenue > 0 ? 
      ((totalRevenue - previousRevenue) / previousRevenue * 100).toFixed(1) : '0.0';
    ...
    const dashboardData = {
      ...
      flightCabinBookings: [
        { cabin: "Economy", bookings: Math.floor(totalBookings * 0.7), revenue: totalRevenue * 0.5, percentage: 70 },
        { cabin: "Business", bookings: Math.floor(totalBookings * 0.25), revenue: totalRevenue * 0.35, percentage: 25 },
        { cabin: "First Class", bookings: Math.floor(totalBookings * 0.05), revenue: totalRevenue * 0.15, percentage: 5 }
      ],
      ...
      systemHealth: {
        ...
        totalQueries: monthlyBookingData.length + topDestinations.length + hotelCityBookings.length,
        responseTime: '< 100ms'
      },
    ...
    sendError(res, 500, 'DASHBOARD_ERROR', 'Failed to generate dashboard summary', error.message);
  }
});

// Real-time metrics endpoint
router.get('/metrics/realtime', requirePermission(Permission.VIEW_DASHBOARD), async (req: AuthenticatedRequest, res) => {
  try {
    const [
      todayBookingsResult,
      todayRevenueResult,
    ...
    const realTimeMetrics = {
      ...
      systemStatus: {
        totalSuppliers: parseInt(systemStatusResult.rows[0]?.total_suppliers || '0'),
        activeSuppliers: parseInt(systemStatusResult.rows[0]?.active_suppliers || '0'),
        avgSuccessRate: parseFloat(systemStatusResult.rows[0]?.avg_success_rate || '0')
      },
    ...
    sendError(res, 500, 'METRICS_ERROR', 'Failed to fetch real-time metrics', error.message);
  }
});

// Booking trends endpoint
router.get('/trends/bookings', requirePermission(Permission.VIEW_DASHBOARD), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const trends = trendsResult.rows.map(row => ({
      ...
      revenue: parseFloat(row.revenue || '0'),
      successRate: row.bookings > 0 ? 
        Math.round((row.successful_bookings / row.bookings) * 100) : 0
    }));
    ...
    sendError(res, 500, 'TRENDS_ERROR', 'Failed to fetch booking trends', error.message);
  }
});
