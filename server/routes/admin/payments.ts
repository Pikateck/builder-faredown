...
router.get('/', requirePermission(Permission.VIEW_PAYMENTS), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    console.log(`💳 Getting payments list - page ${params.page}, pageSize ${params.pageSize}`);
    ...
    const orderBy = params.sort === 'guest' ? 'u.first_name' : 
                   params.sort === 'booking' ? 'hb.booking_ref' :
                   `p.${params.sort}`;
    ...
    const payments = dataResult.rows.map(row => ({
      ...
      amount: {
        ...
        net: parseFloat(row.amount || '0') - parseFloat(row.gateway_fee || '0')
      },
...
router.get('/:id', requirePermission(Permission.VIEW_PAYMENTS), async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
    ...
    const paymentDetails = {
      ...
      amount: {
        ...
        net: parseFloat(payment.amount || '0') - parseFloat(payment.gateway_fee || '0'),
        formatted: formatCurrency(parseFloat(payment.amount || '0'), payment.currency)
      },
      ...
      refund: {
        ...
        formatted: payment.refund_amount ? 
          formatCurrency(parseFloat(payment.refund_amount), payment.currency) : null
      },
      ...
      booking: {
        ...
        formatted: formatCurrency(parseFloat(payment.booking_amount || '0'), payment.currency)
      },
      ...
      // Metadata
      meta: {
        canRefund: payment.status === 'completed' && parseFloat(payment.refund_amount || '0') === 0,
        canPartialRefund: payment.status === 'completed' && 
          parseFloat(payment.refund_amount || '0') < parseFloat(payment.amount || '0'),
        isRefunded: parseFloat(payment.refund_amount || '0') > 0,
        isFullyRefunded: parseFloat(payment.refund_amount || '0') >= parseFloat(payment.amount || '0')
      }
    };
...
router.post('/:id/refund', requirePermission(Permission.PROCESS_REFUNDS), async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
    ...
    // Validate input
    if (!amount || amount <= 0) {
      return sendValidationError(res, ['Refund amount must be greater than 0']);
    }

    if (!reason || reason.trim().length < 3) {
      return sendValidationError(res, ['Refund reason must be at least 3 characters']);
    }
    ...
    if (payment.status !== 'completed') {
      return sendError(res, 400, 'INVALID_STATUS', 'Can only refund completed payments');
    }
    ...
    if (currentRefunded + requestedAmount > totalAmount) {
      return sendError(res, 400, 'EXCESSIVE_REFUND', 
    ...
    const refundRef = refundReference || `REF-${payment.gateway_payment_id}-${Date.now()}`;
...
router.patch('/:id/reconcile', requirePermission(Permission.MANAGE_PAYMENTS), async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
...
router.get('/stats/summary', requirePermission(Permission.VIEW_PAYMENTS), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const successRate = overallStats.total_transactions > 0 ? 
      (overallStats.successful_transactions / overallStats.total_transactions * 100) : 0;

    const stats = {
      overview: {
        totalTransactions: parseInt(overallStats.total_transactions || '0'),
        successfulTransactions: parseInt(overallStats.successful_transactions || '0'),
        ...
        avgTransactionValue: parseFloat(overallStats.avg_transaction_value || '0'),
        successRate: Math.round(successRate * 100) / 100,
        netVolume: parseFloat(overallStats.successful_volume || '0') - parseFloat(overallStats.total_refunded || '0')
      },
      ...
      dailyTrend: dailyVolumeResult.rows.map(row => ({
        ...
        successRate: row.transaction_count > 0 ? 
          Math.round((row.successful_count / row.transaction_count) * 100) : 0
      })),
      
      refunds: {
        refundedTransactions: parseInt(refundStatsResult.rows[0]?.refunded_transactions || '0'),
        totalRefunded: parseFloat(refundStatsResult.rows[0]?.total_refunded || '0'),
        avgRefund: parseFloat(refundStatsResult.rows[0]?.avg_refund || '0'),
        refundRate: overallStats.successful_transactions > 0 ? 
          Math.round((refundStatsResult.rows[0]?.refunded_transactions / overallStats.successful_transactions) * 100 * 100) / 100 : 0
      },
...
router.get('/export/settlement', requirePermission(Permission.VIEW_PAYMENTS), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    let whereClause = 'WHERE p.status = \'completed\' AND p.completed_at >= $1 AND p.completed_at <= $2';
    ...
    const settlementData = settlementResult.rows.map(row => ({
      ...
      fee: parseFloat(row.gateway_fee || '0'),
      refunded: parseFloat(row.refund_amount || '0'),
      netAmount: parseFloat(row.amount || '0') - parseFloat(row.gateway_fee || '0') - parseFloat(row.refund_amount || '0'),
    ...
    if (format === 'csv') {
      // Generate CSV
      const csvHeaders = [
        'Payment ID', 'Order ID', 'Gateway', 'Method', 'Amount', 'Currency', 
        'Fee', 'Refunded', 'Net Amount', 'Completed At', 'Booking Ref', 'Hotel', 'Customer Email'
      ];
      ...
      res.setHeader('Content-Disposition', `attachment; filename="settlement-report-${startDate.toISOString().split('T')[0]}-to-${endDate.toISOString().split('T')[0]}.csv"`);
      res.send(csvContent);
    } else {
      // Return JSON format
      sendSuccess(res, {
        ...
        summary: {
          ...
          totalAmount: settlementData.reduce((sum, row) => sum + row.amount, 0),
          totalFees: settlementData.reduce((sum, row) => sum + row.fee, 0),
          totalRefunded: settlementData.reduce((sum, row) => sum + row.refunded, 0),
          netAmount: settlementData.reduce((sum, row) => sum + row.netAmount, 0)
        },
        reportParams: {
