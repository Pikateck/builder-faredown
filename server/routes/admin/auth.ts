...
router.post('/login', async (req, res) => {
  try {
    ...
    const validAdmins = [
      { email: 'admin@faredown.com', password: 'admin123', name: 'Admin User', role: AdminRole.ADMIN },
      { email: 'superadmin@faredown.com', password: 'super123', name: 'Super Admin', role: AdminRole.SUPER_ADMIN },
      { email: 'finance@faredown.com', password: 'finance123', name: 'Finance User', role: AdminRole.FINANCE }
    ];
...
router.get('/test-token', async (req, res) => {
  try {
    if (process.env.NODE_ENV === 'production') {
      return sendError(res, 403, 'NOT_ALLOWED', 'Test tokens not available in production');
    }
    ...
    sendSuccess(res, {
      ...
      usage: 'Add this token to localStorage as "auth_token" or use in Authorization header'
    });
