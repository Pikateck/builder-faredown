...
const CREDENTIAL_PROFILES = {
  'hotelbeds_sandbox': {
    api_key: process.env.HOTELBEDS_API_KEY || '91d2368789abdb5beec101ce95a9d185',
    api_secret: process.env.HOTELBEDS_API_SECRET || 'a9ffaaecce',
    base_url: process.env.HOTELBEDS_BASE_URL || 'https://api.test.hotelbeds.com'
  },
  'amadeus_sandbox': {
    api_key: process.env.AMADEUS_API_KEY || 'XpQdwZsr8jOmkvaXFECxqp3NgPj8gbBcOv',
...
router.get('/sync-logs', async (req, res) => {
  try {
    ...
    console.log(`📋 Getting sync logs... supplier: ${supplierId}, limit: ${limit}`);
...
router.patch('/:id/status', async (req, res) => {
  try {
    ...
    `, [
      id, 
      JSON.stringify({ from_status: supplier.status, to_status: status })
    ]);
  ...
  try {
    ...
    try {
      ...
      if (supplier.code === 'HOTELBEDS') {
        ...
        throw new Error(`Testing not implemented for supplier: ${supplier.code}`);
      }
    } catch (error) {
      status = 'failed';
...
router.post('/:id/sync', async (req, res) => {
  try {
    ...
    try {
      ...
      if (supplier.code === 'HOTELBEDS') {
        syncResult = await syncHotelbedsData(supplier, destinationCodes, forceSync);
        recordsProcessed = syncResult.hotelsProcessed || 0;
        recordsUpdated = syncResult.hotelsUpdated || 0;
      } else if (supplier.code === 'AMADEUS') {
        syncResult = await syncAmadeusData(supplier, destinationCodes, forceSync);
    ...
    res.json({
      ...
      message: status === 'success' ? 'Sync completed successfully' : 'Sync failed'
    });
...
async function testAmadeusConnection(supplier) {
  ...
  return {
    ...
    version: '2.0',
    environment: supplier.environment,
    testQuery: 'v2/shopping/flight-offers?originLocationCode=BOM&destinationLocationCode=DXB',
    result: 'Connection successful'
  };
}
