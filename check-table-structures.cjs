...
async function checkTableStructures() {
  try {
    ...
    console.log(`- countries.id: ${countriesPK.rows[0]?.data_type || 'NOT FOUND'}`);
    ...
    try {
      const regionsData = await pool.query('SELECT id, name FROM regions LIMIT 3');
      console.log('Regions sample:');
      regionsData.rows.forEach(row => console.log(`  ${row.id} (${typeof row.id}): ${row.name}`));
    } catch (err) {
      console.log('Regions: Error -', err.message);
    }
    
    try {
      const countriesData = await pool.query('SELECT id, name FROM countries LIMIT 3');
      console.log('Countries sample:');
      countriesData.rows.forEach(row => console.log(`  ${row.id} (${typeof row.id}): ${row.name}`));
    } catch (err) {
      console.log('Countries: Error -', err.message);
    }
    
    try {
      const citiesData = await pool.query('SELECT id, name FROM cities LIMIT 3');
      console.log('Cities sample:');
      citiesData.rows.forEach(row => console.log(`  ${row.id} (${typeof row.id}): ${row.name}`));
    } catch (err) {
