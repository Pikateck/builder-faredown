...
async function testBackend() {
  ...
  try {
    ...
    if (usersRes.ok) {
      ...
      console.log(
        "✅ Users found:",
        usersData.total || usersData.users?.length || 0,
      );

      // Check if zubin0478@gmail.com exists
      const zubinUser = usersData.users?.find(
        (u) => u.email === "zubin0478@gmail.com",
      );
      if (zubinUser) {
        ...
        console.log(
          JSON.stringify(usersData.users?.slice(0, 5) || [], null, 2),
        );
      }
    } else {
